...
epid = d["epid"]
ec = np.load(OUT + "/e_cache_manip.npz")
...
smean = Sg.mean(0)
_, _, V = torch.pca_lowrank(Sg - smean, q=KZ, niter=5)
V = V[:, :KZ].contiguous()
ZC = ((Sg - smean) @ V).cpu().numpy()
del Sg
...
def spatial_pca(Sbatch):
    t = torch.tensor(Sbatch, device=dev)
    return ((t - smean) @ Vt).cpu().numpy()
...
np.savez(
    OUT + f"/sp_manip_results_s{SEED}.npz",
