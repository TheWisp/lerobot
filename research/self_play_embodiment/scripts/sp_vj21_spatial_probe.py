...
def encode(ims, bs=48):
    N = len(ims)
    S = np.zeros((N, G * G * 768), np.float32)
...
t0 = time.time()
S = encode(imgs)
print(f"GRID={G} encoded {len(S)} spatial in {time.time() - t0:.0f}s -> {S.shape}", flush=True)
...
print(
    f"object-xy  decode (V-JEPA2.1 spatial 4x4): R2 = {r2(objxy[tem], ridge(Ptr, objxy[trm], Pte)):.3f}  (V-JEPA2 spatial 0.525)",
    flush=True,
)
trg, teg = fe < ne - 4, fe >= ne - 4
Ptr2, Pte2 = pca(S[trg], S[teg], 200)
print(
    f"gripper-xy decode (V-JEPA2.1 spatial 4x4): R2 = {r2(gripxy[teg], ridge(Ptr2, gripxy[trg], Pte2)):.3f}",
    flush=True,
)
