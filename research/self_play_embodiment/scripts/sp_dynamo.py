...
OUT = "/tmp/selfplay_probe"; dev = "cuda"; torch.manual_seed(0)
K = int(os.environ.get("K", "2"))  # unfrozen top blocks
wb = np.load(OUT + "/world_buffer.npz", allow_pickle=True)
imgs = wb["images"]; states = wb["states"].astype(np.float32); W = wb["world"].astype(np.float64)
...
tr = pep < ne - 4
d_act = (states[t1s] - states[ts]).astype(np.float32); amu, asd = d_act[tr].mean(0), d_act[tr].std(0) + 1e-6
...
mean = torch.tensor(proc.image_mean).view(1, 3, 1, 1).to(dev); std = torch.tensor(proc.image_std).view(1, 3, 1, 1).to(dev)
...
tri = np.where(tr)[0]; A = torch.tensor((d_act - amu) / asd, device=dev)
t0 = time.time()
for epoch in range(3):
    np.random.shuffle(tri); li, lf = [], []
    for k in range(0, len(tri), 24):
        b = tri[k:k + 24]
        z0 = enc(ts[b], True); z1 = enc(t1s[b], True)
        li_ = F.mse_loss(inv(torch.cat([z0, z1], 1)), A[b])
        p1 = fwd(torch.cat([z0, A[b]], 1)); lf_ = -F.cosine_similarity(p1, z1.detach(), dim=-1).mean()  # SimSiam stop-grad
        loss = li_ + lf_
        opt.zero_grad(); loss.backward(); opt.step()
        li.append(li_.item()); lf.append(lf_.item())
...
for k in range(0, len(imgs), 64): Zft[k:k + 64] = enc(np.arange(k, min(k + 64, len(imgs))), False).cpu().numpy()
...
def pca(Xtr, Xte, k=200):
    mm = Xtr.mean(0); _, _, Vt = np.linalg.svd(Xtr - mm, full_matrices=False); c = Vt[:k]; return (Xtr - mm) @ c.T, (Xte - mm) @ c.T
def decode(Z):
    Z = Z.astype(np.float64); P, Pt = pca(Z[trm], Z[tem]); return r2(GX[tem], ridge(P, GX[trm], Pt))
print(f"\n=== Fork-1 gate (DINOv2, top-{K} adapted via DynaMo) ===", flush=True)
print(f"eff-rank: frozen {rank(Zfz):.1f}  adapted {rank(Zft.astype(np.float64)):.1f}  (collapse if adapted<<frozen)", flush=True)
print(f"R-gripper-xyz decode (held-out eps): frozen {decode(Zfz):.3f}  adapted {decode(Zft):.3f}", flush=True)
print("  (adapted > frozen, no collapse => Fork-1 adds embodiment info -> run ACT demo-efficiency)", flush=True)
