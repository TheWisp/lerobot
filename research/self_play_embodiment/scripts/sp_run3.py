...
from sp_lib import DinoEncoder, VJepa21Encoder, delta_command, load_emb
...
E_FREE, E_AC, E_INV = (
    ec["e_free"].astype(np.float32),
    ec["e_ac"].astype(np.float32),
    ec["e_invdyn"].astype(np.float32),
)
...
def feats(name, P):
    ...
    if name == "b_free":
        return np.hstack([pr, zc, P["ef"], zg])
    if name == "c_ac":
        return np.hstack([pr, zc, P["ea"], zg])
    if name == "c_invdyn":
        return np.hstack([pr, zc, P["ei"], zg])
...
F_FREE, F_AC, F_INV = (
    load_emb(OUT + f"/f_{SUB}_free.pt"),
    load_emb(OUT + f"/f_{SUB}_ac.pt"),
    load_emb(OUT + f"/f_{SUB}_invdyn.pt"),
)
...
def row(tag, fe):
    return f"  {tag:10s} finalErr {fe.mean():.3f}m | " + " ".join(
...
for nd in DEMOS:
    ...
    for name in conds:
        t0 = time.time()
        fe = rollout(name, fit_policy(name, nd))
        res[(name, nd)] = fe
        log(row(name, fe) + f"  [{time.time() - t0:.0f}s]")
np.savez(
    OUT + f"/sp_{SUB}_results_s{SEED}.npz",
