...
class Vec:
    ...
    def __init__(self, n_envs):
        ...
        leaves = _walk(obs)
        self.imgk = next(
            k for k, v in leaves.items() if v.ndim >= 3 and v.shape[-1] == 3 and v.dtype == np.uint8
        )
...
class DinoEncoder:
    ...
    def encode(self, imgs256, bs=64):
        ...
        for k in range(0, N, bs):
            x = self.proc(images=[Image.fromarray(im) for im in imgs256[k : k + bs]], return_tensors="pt").to(
                DEV
            )
...
class VJepa21Encoder:
    ...
    def encode_both(self, imgs256, G=8, bs=48):
        ...
        N = len(imgs256)
        Mo = np.zeros((N, self.DIM), np.float32)
        So = np.zeros((N, G * G * self.DIM), np.float32)
        for k in range(0, N, bs):
            chunk = np.stack([np.array(Image.fromarray(im).resize((384, 384))) for im in imgs256[k : k + bs]])
            x = torch.from_numpy(chunk).to(DEV).permute(0, 3, 1, 2)[:, :, None].float() / 255.0
            x = (x - self.mean) / self.std
            ...
            So[k : k + b] = (
                hh.reshape(b, G, g // G, G, g // G, C).amax(dim=(2, 4)).reshape(b, -1).cpu().numpy()
            )
        return Mo, So
...
def load_emb(path):
    ...
    f = EmbEnc(ck.get("in_dim", 1408), ck["D"]).to(DEV)
