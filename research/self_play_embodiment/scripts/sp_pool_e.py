...
md = np.load(OUT + "/manip_cache.npz")
Ms = md["M"].astype(np.float32)
Ps = md["states"].astype(np.float32)
eps_ = md["epid"]
ps_pairs = []
...
def make(Mz, Pz, prs):
    z0, z1 = Mz[prs[:, 0]], Mz[prs[:, 1]]
    da = Pz[prs[:, 1]] - Pz[prs[:, 0]]
    return z0, z1, da
...
def train_e(sources):  # list of (M,P,pairs); first source's stats used for norm
    ...
    zmu, zsd = z0.mean(0), z0.std(0) + 1e-6
    amu, asd = da.mean(0), da.std(0) + 1e-6
    Z0, Z1, A = T(z0), T(z1), T((da - amu) / asd)
    n = len(z0)
    ...
    for epc in range(60):
        ...
        for k in range(0, len(tr), 256):
            bi = tr[k : k + 256]
            loss = ((head(torch.cat([f(Z0[bi]), f(Z1[bi])], 1)) - A[bi]) ** 2).mean()
            opt.zero_grad()
            loss.backward()
            opt.step()
    f.eval()
...
def decode(f):
    with torch.no_grad():
        e = f(T(Mr)).cpu().numpy()  # e on our-sim random frames
...
print(
    f"  POOLED        ({len(pr_pairs) + len(ps_pairs)} trans): R2 = {decode(train_e([RP, SP])):.3f}",
    flush=True,
)
