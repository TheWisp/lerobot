...
import os, time, numpy as np, torch, matplotlib
os.environ.setdefault("MUJOCO_GL","egl"); matplotlib.use("Agg"); import matplotlib.pyplot as plt
...
wb=np.load(OUT+"/world_buffer.npz",allow_pickle=True); imgs=wb["images"]
GX=wb["world"][:,6:12].astype(np.float64); J=wb["states"].astype(np.float64)
ep=[tuple(map(int,b)) for b in wb["ep_bounds"]]; ne=len(ep)
@torch.no_grad()
def encode(ims,bs=48,keepgrid=None):
    N=len(ims); M=np.zeros((N,768),np.float32); grids={}
    for k in range(0,N,bs):
        chunk=np.stack([np.array(Image.fromarray(ims[i]).resize((384,384))) for i in range(k,min(k+bs,N))])
        x=torch.from_numpy(chunk).to(dev).permute(0,3,1,2)[:,:,None].float()/255.0; x=(x-mean)/std
        h=enc(x).float()  # (b,576,768)
        M[k:k+len(h)]=h.mean(1).cpu().numpy()
        if keepgrid is not None:
            for j,gi in enumerate(range(k,min(k+bs,N))):
                if gi in keepgrid: grids[gi]=h[j].reshape(24,24,768).cpu().numpy()
    return M,grids
t0=time.time()
...
M,grids=encode(imgs,keepgrid=set(pick)); print(f"encoded {len(M)} @384 in {time.time()-t0:.0f}s -> {M.shape}",flush=True)
...
print(f"  gripper-xyz R2 = {r2(GX[te],ridge(Ptr,GX[tr],Pte)):.3f}   (V-JEPA2 0.78, DINOv2 0.78)",flush=True)
...
gs=np.stack([grids[p] for p in pick]); rgb=pca_rgb(gs)
fig,ax=plt.subplots(len(pick),2,figsize=(6,3*len(pick)))
for r,p in enumerate(pick):
    ax[r,0].imshow(imgs[p]); ax[r,1].imshow(np.array(Image.fromarray((rgb[r]*255).astype(np.uint8)).resize((256,256),Image.NEAREST)))
    for c in range(2): ax[r,c].set_xticks([]); ax[r,c].set_yticks([])
    if r==0: ax[r,0].set_title("image"); ax[r,1].set_title("V-JEPA 2.1 patch-PCA")
plt.tight_layout(); plt.savefig(OUT+"/vj21_pca.png",dpi=110,bbox_inches="tight"); print(f"[ok] vj21_pca.png + vj21_cache.npz",flush=True)
