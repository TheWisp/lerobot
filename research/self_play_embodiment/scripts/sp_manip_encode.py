...
for j, i in enumerate(keep):
    ...
    if (j + 1) % 2000 == 0:
        print(f"  loaded {j + 1}/{len(keep)} ({(j + 1) / (time.time() - t0):.0f}/s)", flush=True)
imgs = np.stack(imgs)
...
def encode(ims, bs=48):
    ...
    S = np.zeros((N, G * G * 768), np.float16)
    for k in range(0, N, bs):
        chunk = ims[k : k + bs]
        x = (
            torch.from_numpy(np.stack([np.array(Image.fromarray(im).resize((384, 384))) for im in chunk]))
            .to(dev)
        ...
        if (k // bs) % 40 == 0:
            print(f"  enc {k}/{N}", flush=True)
    return M, S


t1 = time.time()
M, S = encode(imgs)
print(f"encoded in {time.time() - t1:.0f}s -> M{M.shape} S{S.shape}", flush=True)
