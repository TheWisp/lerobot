...
vec = Vec(8)
img, _ = vec.reset(range(30000, 30008))
enc = VJepa21Encoder()
sim_h = enc.encode(img)  # (8,768) our-sim home


def nn_dist(x, bank):
    return np.linalg.norm(bank - x, axis=1).min()
...
def cos_nn(x, bank):
    n = bank / np.linalg.norm(bank, axis=1, keepdims=True)
