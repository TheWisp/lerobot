...
from sp_lib import Vec, VJepa21Encoder, delta_command, load_emb
...
E_FREE, E_INV = ec["e_free"].astype(np.float32), ec["e_invdyn"].astype(np.float32)
# fit global Jacobian J⁺ (R gripper) from play; expert labels
dj, dg = [], []
for s, e in ep:
    dj.append(states[s + 1 : e].astype(np.float64) - states[s : e - 1])
    dg.append(Rg[s + 1 : e] - Rg[s : e - 1])
...
smean = Sg.mean(0)
_, _, Vv = torch.pca_lowrank(Sg - smean, q=KZ, niter=5)
Vv = Vv[:, :KZ].contiguous()
ZC = ((Sg - smean) @ Vv).cpu().numpy()
del Sg
...
def feats(name, P):
    pr, zc = P["pr"], P["zc"]
    if name == "proprio_only":
        return pr
    if name == "a":
        return np.hstack([pr, zc])
    if name == "noise":
        return np.hstack([pr, zc, P["nz"]])
    if name == "b_free":
        return np.hstack([pr, zc, P["ef"]])
    if name == "c_invdyn":
        return np.hstack([pr, zc, P["ei"]])
    if name == "oracle":
        return np.hstack([pr, P["pg"]])
...
class MLP:
    def fit(self, X, Y):
        ...
        for epc in range(300):
            ...
            if v < best - 1e-5:
                best, bad, bSD = v, 0, {k: x.clone() for k, x in self.net.state_dict().items()}
            else:
                bad += 1
            if bad >= 15:
                break
        self.net.load_state_dict(bSD)
        return self
...
def fit_policy(name, nd):
    ...
    P = dict(
        pr=states[i],
        zc=ZC[i],
        nz=np.random.randn(nd, 64).astype(np.float32),
        ef=E_FREE[i],
        ei=E_INV[i],
        pg=peg[i].astype(np.float32),
    )
    return MLP().fit(feats(name, P), LAB[i])


enc = VJepa21Encoder()
F_FREE, F_INV = load_emb(OUT + "/f_vj21_free.pt"), load_emb(OUT + "/f_vj21_invdyn.pt")


@torch.no_grad()
def emb(m, f):
    return f(torch.tensor(m, device=dev)).cpu().numpy()


def spatial_pca(Sb):
    t = torch.tensor(Sb, device=dev)
    return ((t - smean) @ Vv).cpu().numpy()
...
def grasp_center():
    ...
    for i in range(NG):
        ...
        out[i] = (np.asarray(p.data.xpos[_lf]) + np.asarray(p.data.xpos[_rf])) / 2
    return out
...
def row(tag, b):
    return f"  {tag:13s} minDist {b.mean():.3f}m | " + " ".join(
...
for nd in DEMOS:
    ...
    for name in conds:
        t0 = time.time()
        b = rollout(name, fit_policy(name, nd))
        res[(name, nd)] = b
        log(row(name, b) + f"  [{time.time() - t0:.0f}s]")
np.savez(
    OUT + f"/sp_reach_insim_s{SEED}.npz",
