...
ds_frame0 = to_img(
    ds[int(np.where(ei == 0)[0][0])]["observation.images.top"]
)  # dataset, episode start (home)
ds_frame_mid = to_img(ds[int(np.where(ei == 0)[0][200])]["observation.images.top"])  # mid-episode
vec = Vec(2)
img, _ = vec.reset([0, 1])
sim_frame0 = img[0]  # our sim reset (home)
# step a bit so the arm leaves home
for _ in range(40):
    (img, _), _, _ = vec.step(np.tile(np.zeros(14, np.float32), (2, 1)))
sim_frame_mid = img[0]
fig, ax = plt.subplots(2, 2, figsize=(7, 7))
for a, im, t in [
    (ax[0, 0], ds_frame0, "DATASET frame (home)"),
    (ax[0, 1], ds_frame_mid, "DATASET frame (mid)"),
    (ax[1, 0], sim_frame0, "OUR-SIM reset (home)"),
    (ax[1, 1], sim_frame_mid, "OUR-SIM +40 noop"),
]:
    a.imshow(im)
    ...
    a.set_yticks([])
plt.tight_layout()
plt.savefig(OUT + "/sp_domain_check.png", dpi=110)
print("[ok] sp_domain_check.png", flush=True)
# feature-cloud check
...
print(
    f"  our-sim(mid) nearest dataset-feature dist = {nn:.1f}  (vs dataset typical {dd.mean():.1f})",
    flush=True,
)
