...
tr = ~np.isin(pep, list(range(ne - 4, ne)))
va = ~tr
...
def train(obj):
    ...
    for epoch in range(200):
        ...
        for k in range(0, len(perm), 256):
            bi = perm[k : k + 256]
            if obj == "invdyn":
                pred = head(torch.cat([f(ZT[bi]), f(ZT1[bi])], 1))
                loss = ((pred - A[bi]) ** 2).mean()
            else:
        ...
        if vl < best - 1e-5:
            best, bad, bestSD = vl, 0, {k: v.clone() for k, v in f.state_dict().items()}
        else:
            bad += 1
        if bad >= 12:
            break
