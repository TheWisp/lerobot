...
proprio = d["states"].astype(np.float32)
epid = d["epid"]
IN = M.shape[1]
ne = int(epid.max()) + 1
...
dz = M[t1s] - M[ts]
d_act = proprio[t1s] - proprio[ts]


def stq(x, m):
    return x[m].mean(0), x[m].std(0) + 1e-6


zmu, zsd = stq(M[ts], tr)
dzmu, dzsd = stq(dz, tr)
amu, asd = stq(d_act, tr)
...
def train(obj):
    ...
    if obj == "invdyn":
        ...
        head = nn.Sequential(
            nn.Linear(D + (14 if obj == "ac" else 0), 512), nn.GELU(), nn.Linear(512, IN)
        ).to(dev)
    ...
    for epoch in range(200):
        ...
        for k in range(0, len(perm), 256):
            bi = perm[k : k + 256]
            if obj == "invdyn":
                loss = ((head(torch.cat([f(ZT[bi]), f(ZT1[bi])], 1)) - A[bi]) ** 2).mean()
            else:
        ...
        with torch.no_grad():
            if obj == "invdyn":
                vl = ((head(torch.cat([f(ZT[vaI]), f(ZT1[vaI])], 1)) - A[vaI]) ** 2).mean().item()
            else:
                gin = torch.cat([f(ZT[vaI]), A[vaI]], 1) if obj == "ac" else f(ZT[vaI])
                vl = ((head(gin) - DZ[vaI]) ** 2).mean().item()
        if vl < best - 1e-5:
            best, bad, bSD = vl, 0, {k: v.clone() for k, v in f.state_dict().items()}
        else:
            bad += 1
        if bad >= 12:
            break
    ...
    print(f"  [{obj}] val {best:.4f} (ep {epoch})", flush=True)
    return e_all
