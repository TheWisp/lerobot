...
def analytic_dq(phys, target):  # joint delta (action units≈rad) to move R-gripper toward target
    jacp = np.zeros((3, phys.model.nv)); mujoco.mj_jacBody(phys.model.ptr, phys.data.ptr, jacp, None, bid)
    Jr = jacp[:, rdofs]                                          # (3,6)
...
def run(mode, mean_peg=None):
    ...
    for t in range(H):
        ...
        g = vec.gripper_xyz()[:, 3:]; peg = vec.obj_xyz()[:, :3]
        best = np.minimum(best, np.linalg.norm(g - peg, axis=1))
    return best
# mean peg over the eval seeds (what memorization would aim at)
vec.reset(range(40000, 40000 + NG)); mean_peg = vec.obj_xyz()[:, :3].mean(0)
print(f"\npeg spread across eval seeds: std {vec.obj_xyz()[:,:3].std(0).round(3)} m", flush=True)
b_true = run("true"); b_mean = run("mean", mean_peg)
def line(tag, b): return f"  {tag:18s} minDist {b.mean():.3f}m | " + " ".join(f"SR@{e}={float((b<e).mean()):.2f}" for e in EPS)
print(line("analytic+TRUE peg", b_true), flush=True)
print(line("analytic+MEAN peg", b_mean), flush=True)
print("\n(if TRUE reaches tight & MEAN fails tight -> vision-necessary at a tight threshold, no re-randomization)", flush=True)
