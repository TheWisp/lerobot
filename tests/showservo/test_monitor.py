...
from lerobot.showservo.monitor import _TABLE, Decision, Event, Rung, StageMonitor, State

from .conftest import TARGET_UV, make_team


def _servoing(stage) -> StageMonitor:
    mon = StageMonitor(stage)
...
def _recover_and_resume(mon: StageMonitor) -> None:
...
def test_servoing_never_starts_without_a_bind_certificate(d1_stage):
    mon = StageMonitor(d1_stage)
    mon.feed(Event.START)
...
def test_the_ladder_escalates_in_order_and_skips_regrasp_with_nothing_held(d1_stage):
    mon = StageMonitor(d1_stage)
    mon.feed(Event.START)
    rungs = []
    for _ in range(3):
...
def test_servo_phase_failures_are_ignored_while_still_binding(d1_stage):
    ...
    assert mon.state is State.BINDING and mon.retries_left == d1_stage.budget.retries


def test_regrasp_is_offered_only_when_something_is_actually_held(d2_stage):
    mon = _servoing(d2_stage)
    rungs = []
    for _ in range(4):
...
def test_the_retry_budget_is_honoured_exactly(d1_stage):
    assert d1_stage.budget.retries == 3
    mon = _servoing(d1_stage)
    for _ in range(3):
...
def test_a_recovery_in_flight_does_not_burn_several_rungs_at_once(d1_stage):
...
def test_every_abort_carries_a_failure_class(d1_stage):
    mon = _servoing(d1_stage)
    for _ in range(4):
...
def test_failure_classes_survive_to_the_log(d1_stage, event, expected):
    mon = _servoing(d1_stage)
    for _ in range(4):
        mon.feed(event)
        _recover_and_resume(mon)
...
def test_a_timeout_waiting_for_fission_is_logged_as_a_grasp_failure(d1_stage):
...
def test_a_timeout_on_a_non_grasp_stage_stays_a_timeout(d2_stage):
    mon = StageMonitor(d2_stage)
    mon.feed(Event.START)
    for _ in range(5):
        mon.feed(Event.TIMEOUT)
        mon.feed(Event.RECOVERY_DONE)
...
def test_a_late_event_cannot_undo_a_success(d1_stage):
    mon = StageMonitor(d1_stage)
    mon.feed(Event.START)
    mon.feed(Event.BIND_OK)
...
def test_an_abort_is_final(d1_stage):
    mon = StageMonitor(d1_stage)
    mon.feed(Event.START)
    for _ in range(4):
...
def test_a_stage_that_terminates_during_recovery_still_counts(d1_stage):
...
def test_the_log_carries_the_rungs_that_were_actually_used(d1_stage):
    mon = StageMonitor(d1_stage)
    mon.feed(Event.START)
    for _ in range(2):
        mon.feed(Event.BIND_FAIL)
