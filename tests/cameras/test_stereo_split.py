...
class TestStereoEyesComeFromOneCapture:
    """Both eyes must be read under a single lock acquisition.

    Two separate reads let the grab thread install a new capture between them,
    pairing the left eye of frame N with the right eye of frame N+1 -- a 33 ms
    disparity at 30 fps, handed to the caller as a simultaneous pair. Nothing
    downstream can detect that, which is why it is pinned here.
    """

    class _SwappingLock:
        """A lock whose release advances the frame, as the grab thread would.

        Every acquisition therefore sees a newer pair than the last. An
        implementation that acquires once returns a matched pair; one that
        acquires twice cannot.
        """
        ...
        def __exit__(self, *exc):
            self.generation += 1
            g = self.generation
            self._cam.latest_frame = np.full((4, 4, 3), g, dtype=np.uint8)
            self._cam.latest_frame_right = np.full((4, 4, 3), g + 100, dtype=np.uint8)
            return False

    def _camera(self):
        cam = OpenCVCamera.__new__(OpenCVCamera)
        cam.stereo_split = True
        cam.thread = types.SimpleNamespace(is_alive=lambda: True)
        cam.latest_frame = np.zeros((4, 4, 3), dtype=np.uint8)
        cam.latest_frame_right = np.full((4, 4, 3), 100, dtype=np.uint8)
        cam.latest_timestamp = time.perf_counter()
        cam.frame_lock = self._SwappingLock(cam)
        return cam

    def test_both_eyes_share_one_capture(self):
        cam = self._camera()
        left, right = cam.read_latest_stereo()
        # Encoded so a matched pair always differs by exactly 100.
        assert int(right[0, 0, 0]) - int(left[0, 0, 0]) == 100, (
            f"eyes came from different captures: left={left[0, 0, 0]} right={right[0, 0, 0]}"
        )
