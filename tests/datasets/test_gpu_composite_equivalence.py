...
# ── the whole path is verified by hand, not here ─────────────────────────────
#
# Everything above compares `GpuMaskComposite` against `composite_from_store` on
# synthetic masks, and neither end goes through the reader that feeds it -- which
# is how the GPU path shipped unable to run a single training step (the reader
# stripped the mask rows it needs; see
# tests/datasets/test_saved_masks_training.py, which does run in CI).
#
# A whole-path version belongs here and cannot live here yet: NVDEC refuses this
# module's fixture video outright (`cuvidCreateDecoder ... error 1`), so the test
# would never execute and would read as coverage it does not provide.
#
# Measured by hand instead, on a real 720x1280 two-camera dataset, comparing the
# tensor `DeviceImageSource` hands the model against the loader path's sample for
# the same frames: mean 0.60, max 3 levels, >1 level on 2.8% of pixels. That is
# the decoders disagreeing, not the composite -- the pipeline logs the figure it
# calibrates for exactly this, "agreeing with the CPU decoder to 0.59 levels",
# and the composite's own bound above is max 2.
