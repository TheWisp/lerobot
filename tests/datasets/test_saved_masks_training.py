...
def test_the_reader_keeps_mask_rows_for_whoever_composites_next(masked_dataset_root):
    """Who strips the RLE rows depends on who composites, and it was unconditional.

    On the loader path the reader composites and then drops the rows: a model must
    not receive RLE strings. On the GPU data path the reader decodes nothing --
    that path owns the frames and composites them itself -- and it needs those
    same rows in the batch. Dropping them regardless meant `GpuMaskComposite`
    asked for a column that had already been removed, and the first real training
    run on that path died with `KeyError: 'masks.<camera>'`.

    Nothing caught it because every GPU test drove the compositor or the pipeline
    constructor directly; none went through the reader that feeds them.
    """
    root, repo_id = masked_dataset_root
    ds = LeRobotDataset(repo_id, root=root, return_uint8=True, apply_saved_masks=True)
    mask_keys = [k for k in ds.meta.features if k.startswith("masks.")]
    assert mask_keys, "fixture has no mask columns; the assertions below would be vacuous"

    item = ds[0]
    assert all(k not in item for k in mask_keys), (
        f"the loader path leaked RLE rows into the sample: {[k for k in mask_keys if k in item]}"
    )

    assert not ds.delivers_mask_rows, "a decoding reader must not be advertising the rows"

    # What the GPU path does before iterating.
    ds.set_video_decoding(False)
    assert ds.delivers_mask_rows, (
        "the named invariant disagrees with the reader: `delivers_mask_rows` is what "
        "the handoff in image_source asserts, so it has to track the actual behaviour"
    )
    item = ds[0]
    assert all(k in item for k in mask_keys), (
        "the reader dropped the mask rows while not decoding; the GPU composite has "
        f"nothing to composite from — missing {[k for k in mask_keys if k not in item]}"
    )
