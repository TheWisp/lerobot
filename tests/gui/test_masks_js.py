...
``mask_frame_fixture.json`` does the same one level up, for the enabled flag
rather than the run lengths: its ``expect_drawn`` is what Python's
``decode_frame`` returns, so JS is compared against what the trainer actually
composites. A disagreement there paints a mask the trainer ignores, which reads
as the mute not having worked.

Regenerate both with ``tests/gui/regen_mask_fixture.py`` if the encoding ever
changes; a stale fixture would let the two drift apart silently.
"""
...
def test_frame_fixture_covers_both_flag_forms():
    """Pinned here too, because the node test is skipped without node."""
    import json

    cases = json.loads((Path(__file__).parent / "mask_frame_fixture.json").read_text())
    by_name = {c["name"]: c for c in cases}
    assert {"both_enabled", "one_disabled", "all_disabled"} <= set(by_name), by_name.keys()
    # The forms the encoder never writes but every reader must accept.
    assert {"explicit_one", "explicit_true"} <= set(by_name), by_name.keys()
    assert by_name["explicit_one"]["expect_drawn"] == by_name["both_enabled"]["expect_drawn"]
    # And the case that must differ, or "JS agrees with Python" is vacuous.
    assert by_name["one_disabled"]["expect_drawn"] != by_name["both_enabled"]["expect_drawn"]
