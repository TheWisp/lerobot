...
- [x] **[RLT] Measured on-robot inference delay with the 4L d=2048 encoder.** Analyzed `outputs/rlt_online_v2_widened/chunk_compare.jsonl` (3997 RLT-active records over 70 eps). Per-stage timings (mean / median / p95 / max in ms): `rl_tok_ms` 3.7 / 2.8 / 11.3 / 35.3 (the widened encoder), `enc_obs_ms` 12.2 / 10.0 / 24.4 / 80.9, `s1_denoise_ms` 39.8 / 38.2 / 58.7 / 149.8 (largest contributor), `rlt_actor_ms` 0.6 / 0.3 / 1.3 / 52.3, `obs_to_infer_ms` 36.9 / 32.9 / 59.9 / 513.5 (mostly inter-frame wait, not compute). End-to-end `total_delay_ms` 95.9 / 91.3 / 131.5 / 569.5. RTC budget at 30Hz with `rtc_max_delay=6` is ~200ms; only 1.4% of inferences exceed it (vs 33.8% above 100ms). The widened encoder cost is negligible — d=2048 ships fine on real hardware.
...
- [ ] **[HIGH][RLT] Two-stage replay buffer for IGNORE invariant.** Currently transitions go straight into the live `ReplayBuffer` as the inference thread runs; on IGNORE the main thread truncates back to `ep_start_size`. Between add and truncate, grad updates have already sampled the doomed transitions — small but non-zero training-weight drift on episodes the operator wanted to throw out. Make the invariant structural: a `StagingArea` (per-episode, cleared at episode start) accumulates transitions, while grad updates ONLY sample from a separate `CommittedBuffer`. At episode end: success/abort/fail → commit (move staging to committed), IGNORE → discard (clear staging without committing). Cost: ~250 grad steps of staleness per episode boundary (transitions from episode N only become trainable in episode N+1) — acceptable since the BC anchor + slow target update easily absorb that lag, and we don't know an episode's outcome until it ends anyway. Map onto the existing `lifecycle.begin()` / `end_episode()` hooks. Lockable as a unit-test invariant: "grad sampling never returns a transition from the active episode's staging area." ~150 LOC + tests.
