...
- [ ] **[RLT] Measure real RLT inference delay** — offline pipeline is now correct; needs a robot run with the new per-stage dump to decide next step. Script: `python scripts/rlt_token_probe.py` offline reconstruction probe decides if Phase-1 retrain is needed (< 10% rel RMSE = skip).
- [ ] **[RLT] Actor slice `[D:D+C]` vs `[0:C]`** — depends on the measurement above. Decision pending.
- [ ] **[RLT] Retrain Phase 2 (fresh)** after robot run. The buggy checkpoint is preserved as `outputs/rlt_online/latest.buggy_pre_fixes_20260423/` and the associated dump as `chunk_compare.jsonl.pre_fixes_20260423` — delete when confident they're not needed.
