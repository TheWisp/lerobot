...
- [ ] **[HIGH][RLT-metrics] Misleading window naming in summary fields** — `success_rate`, `autonomous_rate`, and `intervention_rate` are all computed over the **last 20 episodes**, while `total_episodes` / `total_successes` / `total_autonomous` are all-time. They're presented under one summary banner, so numbers don't appear to reconcile (e.g. "82 episodes, 35 successes, success_rate 70%" — 35/82 ≈ 43%, but 14/20 = 70%). The earlier observation of "intervention_rate=0.0 despite many interventions" in the v1 run was the same issue — the last 20 episodes of that run had no interventions, so the rolling-20 was correctly 0. Fix options: (a) rename to make windows explicit (`success_rate_rolling20`, `autonomous_rate_rolling20`, etc.), (b) emit BOTH all-time and rolling-20 versions and let the consumer decide, or (c) standardize everything to all-time and let the GUI render its own rolling chart. (b) is least disruptive to existing consumers.
