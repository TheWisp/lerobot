...
### High priority
- [ ] **[CRITICAL] RLT state normalization mismatch between training and inference.** `FlowMatchingDataset` stores z-score-normalized `observation.state` (train.py:116,138), so Phase-1 RL token encoder training sees normalized state in the context. At RLT inference, `obs_to_s1_batch` (s1_process.py:182) writes RAW joint values (~±90°, σ≈24) and `s1_inference.py:660` calls `encode_observations` before `predict_action_chunk` normalizes (model.py:484). Result: every z_rl produced in online training and deployment has been computed from OOD state (~24× overscale). The actor and critic adapted to this garbage z_rl, which explains the 46% token reconstruction error and the "kind of worked" observation. Fix: normalize state in `obs_to_s1_batch` or at top of inference block before `encode_observations`. Drop the old actor/critic/replay checkpoint after fixing — they're unrecoverable. Retrain Phase 2 from scratch. Guardrail test required: given identical batch, `encode_observations` output must match between training-path and inference-path.
- [ ] **[RLT] Image preprocessing parity between training and inference — and make it policy-owned.** Current fix scope: flow_matching S1 (what RLT uses) trains with `TF.resize(bilinear, antialias=True)` (train.py:155-158) but inference uses `cv2.resize(INTER_LINEAR)` no-antialias (s1_process.py:175-178). Broader fix: preprocessing should be a method on the S1 policy (`policy.preprocess_image(img) -> tensor`), because ACT-VLM S1 uses a different pipeline (`F.interpolate(bilinear)` no-antialias, train_act_vlm.py:421-423) AND supports both DINOv2 and ResNet18 backbones. Hard-coding in `obs_to_s1_batch` means any new policy type silently mismatches. Immediate fix: switch flow_matching inference path to torchvision `bilinear + antialias`. Medium-term: push preprocessing into each policy class so inference calls `self._policy.preprocess_image()` — permanently eliminates this class of bug.
- [ ] **[RLT] Actor forward dtype mismatch.** Training runs actor in fp32 (no autocast). Inference wraps actor forward inside `torch.autocast(bfloat16)` at s1_inference.py:595. Small-MLP numerical drift between train/infer. Fix: pull actor call out of the autocast block and run fp32, or add matching autocast around training updates in `_rlt_gradient_updates`.
- [ ] **[RLT] Duplicate `encode_observations` call.** s1_inference.py:660 calls it before `predict_action_chunk`, which internally calls it again at flow_matching/model.py:389. ~11ms of DINOv2 forward wasted per inference (most of the current ~13ms "RLT overhead"). After bug #1 is fixed the two calls produce identical output, so they can be consolidated — expose a `context` kwarg on `predict_action_chunk` and share.
- [ ] **[RLT] Measure real RLT inference delay before deciding on actor slice change.** Current dump has `rlt_enc_ms` + `rlt_post_ms`, but `rlt_enc_ms` is dominated by the duplicate `encode_observations`. After removing the duplicate, re-measure; that tells us how much genuine RLT overhead there is. Only then decide whether changing the actor slice from `[0:C]` to `[D:D+C]` (skipping the prefix territory that gets clamped by S1) is worth the retraining churn.
- [ ] **[RLT] Actor slice should be `[D:D+C]` instead of `[0:C]`.** Contingent on the measurement above. Actor currently rewrites prefix positions that the robot skips via `t_origin` — but the modified prefix ends up in the next inference's RTC prefix slot ~22% of the time (observed), feeding S1 an off-distribution kinked prefix. Fixing this requires: (a) changing actor input/output slice at inference, (b) matching change in what gets stored in replay buffer, (c) retraining actor/critic. Group with bug #1 retraining.
- [ ] **[RLT] Add guardrail tests for training/inference pipeline equivalence.** The state normalization bug should have been caught by a parity test. Add: (1) `encode_observations` output matches between training-path batch and inference-path batch given identical logical inputs; (2) image-resize output matches between torchvision and cv2 paths within tight tolerance OR unify on one path; (3) actor output at deploy matches output at training-time for same inputs (fp32 vs bf16 parity check).
- [ ] **[RLT] Add per-inference-stage timing + sanity metadata to `chunk_compare.jsonl`.** Current fields (`rlt_enc_ms`, `rlt_post_ms`, `total_delay_ms`) are too coarse. Split into: `obs_to_infer_ms`, `enc_obs_ms` (DINOv2 only), `rl_tok_ms` (RL token encoder only), `s1_denoise_ms` (predict_action_chunk), `rlt_actor_ms` (actor forward only), `dump_ms`. Plus stage-specific sanity: e.g., first state value post-normalization (to catch future normalization-bug regressions from the dump alone).
