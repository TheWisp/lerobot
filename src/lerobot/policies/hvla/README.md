...
### High priority
- [ ] **RLT episode numbering off-by-one, migrate to 0-indexed**. `rlt_state["episode"]` is incremented at episode END, so mid-episode reads (dump file's `ep` field, warmup check `episode <= warmup_episodes`) see `current_ep - 1`. The log shows post-increment value so it looks 1-indexed from outside ("RLT ep1" is the first ever). Dataset episodes are 0-indexed. Fix: move increment to episode start AND shift external numbering to 0-indexed (first ever ep = "ep0"). Also renumber existing artifacts (train.log "RLT epN" → "RLT ep(N-1)", `training_state.pt["episode"]` -= 1, `metrics.json` top-level `episode` -= 1). The `chunk_compare.jsonl` is already correct for the new scheme by coincidence. Also check `warmup_episodes=10` actually gives 10 warmup eps (currently gives 11 due to the same off-by-one).
