...
- [ ] **[RLT] Image preprocessing parity between training and inference.** Training uses `torchvision.transforms.functional.resize(bilinear, antialias=True)` on float[0,1] tensors (train.py:151-158). Inference uses `cv2.resize(..., INTER_LINEAR)` on uint8 then /255 (s1_process.py:175-178). Antialiasing difference produces different high-freq content; DINOv2 is sensitive to it. Fix: switch inference to torchvision resize (same ordering as training).
