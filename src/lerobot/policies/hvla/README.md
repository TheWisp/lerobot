...
- [ ] **[HIGH][RLT-metrics] `episode_lengths_s` always 0.0** — the per-episode duration is correctly computed and printed in `train.log` ("RLT epN: ASSISTED ... | 13.4s"), but the corresponding entry in `metrics.json`'s `episode_lengths_s` series is 0.0 for every episode. The duration isn't reaching the metrics layer's `record_episode` call. Throughput and any duration-derived chart on the GUI dashboard are unusable until this is fixed.
- [ ] **[HIGH][RLT-metrics] `intervention_rate` reads 0.0 despite real interventions** — top-level aggregation bug. The per-episode `episode_autonomous` series is populated correctly (False entries for ASSISTED episodes), but the headline `intervention_rate` is computed wrong (or off a different field) and shows 0.0. Hides the "user intervened often" signal in any post-hoc summary. Likely a missing-divisor / wrong-numerator at snapshot time.
- [ ] **[HIGH][RLT-metrics] Mixed-window naming in summary fields** — `success_rate` is computed over the last 20 episodes; `autonomous_rate` and `intervention_rate` are all-time. They're presented under one summary banner, leading to numbers that don't add up (e.g. "82 episodes, 35 successes, success_rate 70%" — 35/82 ≈ 43%). Either standardize all summary fields to the same window, or rename to make the windowing explicit (`success_rate_rolling20`, `autonomous_rate_alltime`).
...
- [ ] **[RLT] Actor slice `[D:D+C]` vs `[0:C]`** — still deferred, NOT implemented for the first Phase-2 run. Decision rationale: with the pipeline now fixed and the widened (4L d=2048) encoder trained, we run Phase 2 as-is (`actor rewrites frames [0:C]`) and use the per-inference dump to see whether the actor/S1 boundary crossing actually hurts learning. If Phase 2 autonomous success rate climbs above S1 baseline, the slice mismatch wasn't blocking. If it stalls AND the dump shows large boundary jumps (`actor[C-1]` far from `ref[C]`), implement `[D:D+C]` and retrain. Earlier analysis observed ~22% of RTC prefixes crossed the boundary with the buggy encoder; needs re-measurement with the corrected pipeline (same analysis script can reuse the per-stage dump).
...
- [ ] **[RLT] Atomic checkpoint saves** — `latest/` is written file-by-file (`actor.pt`, `critic.pt`, `critic_target.pt`, `training_state.pt`, `replay_buffer.pt`), so an exception or crash mid-save can leave a torn checkpoint where actor and critic are from different gradient steps. Mitigation: write each file via `tmp + os.replace` (matches the metrics.json save pattern), or write the whole checkpoint into `latest.new/` then dir-rename for atomicity. Mid-priority — per-10-ep `ep_N/` snapshots are the current safety net, and they're written at clean episode-end points so they don't suffer from this.
- [ ] **[HVLA] stderr missing from subprocess run log** — `outputs/hvla_runs/run_*.log` uses a `logging.FileHandler` on the root logger, which only captures messages that flow through Python's logging module. Python's default uncaught-exception handler writes tracebacks directly to `sys.stderr`, bypassing logging → the crash traceback ends up only in the GUI output panel (via stderr-pipe) and not in the persisted log file. Post-hoc debugging (looking at yesterday's crash) misses the traceback. Fix: install a `sys.excepthook` in `run_s1` / `launch.py` that routes uncaught exceptions through `logger.error(..., exc_info=...)` so they land in the file log. Mid-priority.
