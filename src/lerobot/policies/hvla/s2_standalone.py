...
def main():
    ...
    # hardcode-ok: pi05 S2 view names/order -- external checkpoint convention, not ours (see S2_CAM_KEY_MAP)
    parser.add_argument(
        "--s2-image-keys",
        nargs="+",
    ...
    logger.info(
        "Creating shared memory (latent: '%s', images: '%s*')...",
        SharedLatentCache.SHM_NAME,
        SharedImageBuffer.SHM_PREFIX,
    )
