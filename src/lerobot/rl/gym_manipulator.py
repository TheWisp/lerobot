...
def step_env_and_process_transition(
    ...
    for key, value in action_info.items():
        if isinstance(key, TeleopEvents):
            new_info[key] = value
    # fork: the enum-only copy above is also the fix for the one-step stale lag
    # the fork patched on the pre-refactor code — the action-processor's info is
    # carried forward from the previous iter's transition, so its string keys
    # ("is_intervention" / "teleop_action" / "rerecord_episode") reflect the
    # previous step. Letting them win manifests as recorded actions being
    # all-zero even when the user actively intervenes. Copying only the
    # (freshly updated) `TeleopEvents` enum keys keeps gym-hil's current-step
    # string keys authoritative.
