"""
Keyboard teleop using ikpy-based numerical IK (no table required).

Each tick:
    target_ee accumulates step·direction for held keys (anchored to press start)
    ikpy.inverse_kinematics(target_ee, initial=current motors) -> joint solution
    motors slew toward joint solution (max_relative_target * 2 per tick)
    target_q is latched between active commands (anti-sag)

Usage:
    .venv/bin/python -m lerobot.robots.so107_description.teleop_keyboard_ikpy \\
...
from ..kinematics import MOTOR_NAMES, RIGHT_ARM_MAP
from ..kinematics_ikpy import So107IkpyKinematics
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=float,
        default=80.0,
        help="max EE travel from press_anchor in a continuous press",
    )
    parser.add_argument(
        "--smoothing-alpha",
        type=float,
        default=0.3,
        help="EMA smoothing for target_q. Lower = smoother (more lag). 1.0 disables smoothing.",
    )
    ...
    kin = So107IkpyKinematics(joint_map=RIGHT_ARM_MAP)
    ...
    log_path = Path(f"/tmp/so107_ikpy_{dt.datetime.now():%Y%m%d_%H%M%S}.csv")
    ...
    status["info"].set("ikpy IK. WASD QE move, [] gripper, R reset, Esc quit")
    ...
    def tick() -> None:
        ...
        try:
            ...
            # 1. Update target_ee from held keys (anchored to press start).
            cmd_dir = np.zeros(3)
            ...
            # 2. IK only when there's an active command. Otherwise hold last target_q.
            ik_status = "idle (holding target_q)"
            if cmd_norm > 1e-6:
                target_T = current_T.copy()
                target_T[:3, 3] = target_ee
                ik_motors, ik_err_mm = kin.ik_to_motors(
                    mp,
                    target_T,
                    orientation_mode=None,  # position only
                )
                # EMA-smooth the latched target_q toward IK result. Low alpha
                # suppresses tick-to-tick SLSQP jitter; full update (alpha=1)
                # gives raw IK.
                a = max(0.0, min(1.0, args.smoothing_alpha))
                for n in MOTOR_NAMES:
                    if n != "gripper":
                        target_q[n] = a * ik_motors[n] + (1 - a) * target_q[n]
                ik_status = f"err {ik_err_mm:.2f}mm  alpha={a:.2f}"
            else:
                ik_err_mm = 0.0
            target_q["gripper"] = gripper_target

            # 3. Slew motors toward target_q with safety clamps.
            new_q = {}
            for n in MOTOR_NAMES:
                d = target_q[n] - mp[n]
