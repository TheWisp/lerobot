...
Prints four signals -- training loss, best-of-K distance, same-frame sampling
spread, between-frame spread -- and the findings that follow from them;
``--json out.json`` writes the full structure for a sweep or a test.
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--samples", type=int, default=4, help="samples per frame; >1 enables the spread signals"
    )
    ...
    print(f"dataset    : {report.dataset}  ({len(report.frames)} frames x {args.samples} samples)")

    loss = "n/a" if report.training_loss is None else f"{report.training_loss:.4f}"
    print(f"\ntraining loss on these frames : {loss}      <- did it fit at all")
    print(f"best-of-K distance (action std) : {report.best_of_k:.4f}   <- covers the demonstrated mode?")
    print(f"spread, same frame resampled    : {report.sample_spread:.4f}   <- mode switching risk")
    print(f"spread, across frames           : {report.between_frame_spread:.4f}   <- responds to input?")
    print(
        f"  (recorded actions vary by      {report.recorded_spread:.4f} — the scale to judge those against)"
    )

    worst = max(report.frames, key=lambda f: f.best_of_k)
    print(
        f"\nworst frame: episode {worst.episode_index} frame {worst.frame_index} "
        f"(best-of-{worst.samples} {worst.best_of_k:.4f})"
    )
    print("  per-joint:", ", ".join(f"{v:.3f}" for v in worst.best_of_k_per_joint))
    print("  closest prediction[0]:", ", ".join(f"{v:.3f}" for v in worst.predicted_first))
