...
def train(cfg: TrainPipelineConfig, accelerator: "Accelerator | None" = None):
    ...
    # fork: a resumed PEFT policy carries ``use_peft=True`` in its config even
    # when ``--peft`` is not passed again, so honour both sources. (``cfg.policy``
    # may be None for reward-model training, hence the getattr.)
    is_peft = cfg.peft is not None or getattr(cfg.policy, "use_peft", False)
    ...
    if is_peft:
        ...
        if isinstance(policy, PeftModel):
            ...
            # fork: Resuming PEFT training — the model was loaded with adapters,
            # so ensure adapter parameters are trainable (may have been saved as frozen)
            ...
            peft_cli_overrides = dataclasses.asdict(cfg.peft) if cfg.peft is not None else {}
            policy = policy.wrap_with_peft(peft_cli_overrides=peft_cli_overrides)
    else:
        # fork: Non-PEFT — apply selective parameter freezing for Pi0.5 to reduce VRAM usage on single GPU
