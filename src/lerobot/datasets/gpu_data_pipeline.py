...
class GpuImagePipeline:
    ...
    def __init__(
        ...
        # Saved masks are this branch's feature, so this is where the data path
        # learns to composite them. The layer below ships decode and resize only
        # and refuses a masked dataset outright; that refusal is replaced here.
    ...
    def prepare(self, batch: dict[str, Any], timed: bool = False) -> dict[str, torch.Tensor]:
        ...
        for cam in self.cameras:
            ...
            # Compositing is this layer's addition. It sits between decode and
            # conversion because the mask applies to the uint8 frame: doing it
            # after the resize would composite against interpolated pixels.
