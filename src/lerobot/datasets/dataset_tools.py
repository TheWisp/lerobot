    ...
    if is_scalar_shape:
        ...
        if isinstance(value, (list, tuple, np.ndarray)):
            ...
            if arr.size != 1:
                ...
                # ``raise`` short-circuits; comment kept so future maintainers understand
                # why we don't fall through to the scalar coercion below.
            scalar = arr.item()
        else:
            scalar = value
        if dtype == "bool":
...
def set_feature_values(
    ...
    *,
    ...
    :func:`modify_features`. Affected ``data/chunk-*/file-*.parquet``
    shards are rewritten with the listed cells overwritten; per-episode
    ``stats/<feature>/*`` columns in ``meta/episodes/*.parquet`` are
    recomputed for every episode whose frames were touched. Videos and
    ``info.json`` are NEVER modified by this function.
    ...
    Raises:
        ValueError: If any edit references a missing feature, an image/video
            feature, an out-of-range frame range, or a value that doesn't
            match the feature's dtype/shape.
