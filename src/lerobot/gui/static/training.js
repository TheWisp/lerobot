    ...
    <div id="training-image-local" style="display:none;">
      ...
      <details class="training-image-advanced">
        ...
        <label class="training-field training-field-bool">
          ...
          <span class="training-field-hint">Ignore Docker's reusable build layers. Use only for troubleshooting; this can reinstall Torch/CUDA dependencies and download several GB.</span>
        </label>
      </details>
...
async function trainingBuildImage() {
  ...
  const forceFullInput = document.querySelector(
    '#training-start-form input[name="image_force_full_rebuild"]'
  );
  ...
  try {
    ...
    _trainingBuildNote = {
      text: _trainingForceFullRebuild
        ? "Full rebuild… ignoring Docker layer cache; dependencies may be downloaded again."
        : "Incremental build… reusing local Docker layers when valid.",
      color: "#e5c07b",
    };
    _trainingBuildTail = [];
...
async function trainingCheckBuildStatus() {
  ...
  if (!failed) {
    // A forced rebuild is a one-shot diagnostic action. Return subsequent
    // builds to the safe, fast incremental default.
    _trainingForceFullRebuild = false;
    const forceFullInput = document.querySelector('input[name="image_force_full_rebuild"]');
    if (forceFullInput) forceFullInput.checked = false;
