...
Every upload — regardless of size — runs through HF's pull-request mechanism: we create a PR (which auto-creates a `refs/pr/<num>` branch on the repo), `upload_large_folder` writes to that PR branch, then `super_squash_history` collapses the per-batch commits into one, and the PR merges into `main` as a single clean commit. This is what gives us **unconditional atomicity** on the consumer-facing side: `main` either reflects the entire upload or none of it, never a partial state — even for multi-hundred-GB uploads spanning hours.
...
| **`super_squash_history` fails**                                                        | Untouched                                            | All upload commits intact (unsquashed)                                                                         | Same                                                                                           | Same `main` as before                                               |
...
**The cross-cutting invariant: `main` only ever changes via `merge_pull_request`, and `merge_pull_request` is atomic — it either updates main to the squashed PR tip in one go, or doesn't.** Every other operation (PR creation, upload, squash, status change) touches only the PR branch.
