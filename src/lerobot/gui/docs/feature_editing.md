...
| `task_index` (with `meta/tasks.parquet`)          | not on timeline — per-episode, so the decoded `task` string shows in the Inspector's episode-only section | **read-only, but not immutable.** Upstream supports changing it: `modify_tasks` is exported from `lerobot.datasets` and wired into `lerobot-edit-dataset`. It is a metadata operation — dedupe and reindex the string table, rewrite `total_tasks`, rewrite `task_index` on every row — while this pipeline stages per-frame values over a `[from, to)` range, so staging it that way would desynchronise `meta/tasks.parquet` from `info.json` or mint duplicate strings. Editing from the GUI is tracked in [#125](https://github.com/TheWisp/lerobot/issues/125). |
