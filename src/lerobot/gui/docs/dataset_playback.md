...
The player hands each paint the chunk's rows for the frame; the mask layer
draws them with its palette and label visibility, and the recipe's treatment —
tint, blur, random — is applied on the decoded frame per pixel under the mask,
using the effective recipe the mask layer already fetches with
`episodes/{ep}/masks/status`, with outlines over it as the JPEG composite view
draws them. What is drawn approximates the library's compositor
([Non-goals](#scope)).
