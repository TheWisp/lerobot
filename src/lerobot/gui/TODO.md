...
- [ ] **Live pose tracking → use FoundationPose (feed-forward), NOT iterative render-compare.** We prototyped silhouette analysis-by-synthesis (fit the GLB pose so its rendered mask matches SAM3's) — works (synthetic recall ~1.0) but it's the **legacy iterative paradigm**: scipy Nelder-Mead re-renders the mesh ~50–150×/frame (the "~100 ms"), and silhouette-only fitting is ambiguous for symmetric objects (IoU caps ~0.85 even synthetically; cylinder/ring is worst-case). SOTA replaced this with **feed-forward** models. For our exact case (known mesh + live 6-DoF + occlusion completion) the targeted answer is **[FoundationPose](https://github.com/NVlabs/FoundationPose)** (NVIDIA, CVPR'24): model-based mode takes mesh (✓ SAM3D) + RGB (✓) + initial SAM3 mask (✓) + RealSense depth (✓) → real-time 6-DoF **tracking**, occlusion-robust via the model prior (= the amodal "hidden geometry"). Render-and-compare done right (its learned predecessor is [MegaPose](https://arxiv.org/pdf/2212.06870)). Keep SAM3 for the mask + the URDF viewer for display; FoundationPose just replaces the pose engine.
  - **Install scope on the 5090 (sm_120):** isolated venv like SAM 3D Objects. Deps: PyTorch (cu128 for sm_120, not the repo's default cu124), **PyTorch3D + nvdiffrast + Kaolin built from source** — all of which we **already cracked for Blackwell** in [[reference_sam3d_blackwell_install]] (big de-risk; clone that env as the base), plus FoundationPose's own `mycpp/mycuda` CUDA extension (`build_all.sh`, target sm_120) and two weight files (refiner + scorer, Google Drive, ~1 GB). Moderate effort, mostly already-solved builds.
