...
- [ ] URDF / 3D trajectory views
  - [ ] **Chunk-source playback in the URDF viewer.** Action chunks from a policy (or a debug-model running alongside teleop) come through `/api/run/urdf-viz?source=...` as `frames[]` of length N + `fps`. The viewer applies `frames[0]` only today. Need: (a) producer-side pause/snapshot so a captured chunk doesn't fight live motion, (b) scrubber UI in the URDF tile to step through `frames[1..]` at the producer's `fps`. Design notes in `gui/docs/urdf_viz.md` ("Future sources").
  - [ ] **EE-trajectory line as an always-live alternative.** Reduced version of chunk playback: render the chunk as a static polyline through 3D space (FK(joints) of each frame). Always safe to draw alongside live motion since it's a path, not a moving robot. Less rich than the playback above; doesn't need the pause hook.
...
- [Mid] **`scripts/gui/gui_compose.py` — post-process toolkit.** Side-by-side composer (lift the implementation already inline in `mcp/docs/proofs_e2e.py`), crop-to-window, extract-frame-at-time. Each is a 5-line ffmpeg/Pillow wrapper, but canonical CLIs mean skill markdown reads "for before/after, call `gui_compose.py side_by_side a.png b.png out.png`" instead of teaching ffmpeg flags every conversation.
