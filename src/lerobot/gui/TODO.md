...
- [ ] **SAM3-video streaming OOM — UNRESOLVED; needs the right API usage (expert request filed: `SAM3_VIDEO_STREAMING_OOM.md`).** `Sam3VideoInferenceSession` stores per-frame state (`output_dict_per_obj[obj]["non_cond_frame_outputs"]`, `processed_frames`) and **never evicts** it → GPU grows **~6.8 MB/frame, linear, NO plateau** (measured: 1.85 → 5.22 GB over 385 frames; 8.9 GB by 1050). **What does NOT work (all measured 2026-06-23):** (1) `inference_state_device="cpu"` — still linear (2.8 → 8.9 GB / 1050 frames; only ~20% shifts off-GPU). (2) **Manually pruning** old `non_cond_frame_outputs`/`processed_frames` (keep last 32) bounds GPU (+0.3 GB) but **BREAKS the detector→track association**: detections re-add as new objects (1→4 instances over 13 s on a moving scene) and the track goes **stale after ~10 s and never recovers**. An earlier "prune is lossless" claim here was WRONG — it was measured on a near-static clip; under motion the prune corrupts tracking. (3) An output-side single-instance lock on top of the prune hides the extra IDs (0 ID-switches offline) but live still goes stale (masks the symptom while the prune keeps corrupting underneath). `max_num_objects=1` caps instances but doesn't touch the per-frame memory growth. **Reverted to the honest unbounded baseline** (tracks correctly, OOMs over time). Real fix likely: a supported eviction/window for the session, correct device placement, or **`Sam3TrackerVideoModel`** (geometric seed-once-and-propagate) instead of the concept model for single-object tracking. **Workaround today:** single camera + short sessions; watch the VRAM label.
- [x] **Leak audit (2026-06-23):** ran every integrated model for 80 steady-state frames (warmup past the fill-ramp), watching `torch.cuda.memory_allocated` — **all flat, no leaks**: the 6 stateless adapters (grounding_dino, dino_features, depth_anything, sam2_mask, sam3_tracker, sam3), `cotracker3` (bounded `_buf`/`_trails`, capped online window), and the FoundationPose worker (`track_one`×80 + `register`×10 flat). `sam3_video` is the one unbounded grower — inherent to the streaming session (the Problem above), not a leak in our code. Re-run this check when adding a new debug-vision adapter.
