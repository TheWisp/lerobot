...
- [High] **RLT reward hotkeys have no transport on a GUI-launched run.** The stdin control channel carries `exit_early` / `rerecord_episode` / `stop_recording` — episode _flow_. RLT's terminal-reward keys are a different vocabulary: `R` signals SUCCESS (+1) and LEFT signals ABORT (`cfg.abort_reward`, default −1.0), and `s1_process` delivers them by wrapping `listener.on_press`. That is a `pynput` attribute, so the hook is already inert under the terminal backend, and the GUI sets `LEROBOT_KEYBOARD_LISTENER=0` for subprocesses it launches — which means no keyboard backend at all, and no reward path. The run still executes; the operator simply cannot mark an episode succeeded or aborted, so every trajectory lands with whatever the default terminal reward is. Fixing it means extending the stdin vocabulary with the reward commands (both ends: `_CONTROL_COMMANDS` in `gui/api/run.py` and `_STDIN_COMMAND_TO_CONTROL` in `keyboard_input.py`, which a test now pins as equal) and giving `s1_process` a supported hook instead of monkeypatching `on_press`. Worth doing before the next RLT data-collection session driven from the GUI.
