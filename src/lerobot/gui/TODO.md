...
- [High] **GUI server is holding `/dev/video*` FDs that block CLI teleop.** Reproduced 2026-05-11: after a teleop subprocess crashed (camera disconnect on `/dev/video2`), the GUI's launch of a follow-up teleop failed with `Failed to open OpenCVCamera(/dev/video10)`. `fuser /dev/video10` reported the **lerobot.gui server itself** (PID 2800036, NOT a subprocess) holding FD 21 against video10. A minute later `lsof -p <gui_pid>` no longer showed it, so the grip is transient — either a preview-snapshot path that takes seconds to release, or a `cv2.VideoCapture` whose `release()` is lazy until GC. Either way it's wrong: V4L2 device nodes are exclusive-open, so any second-long grip can lock out a concurrent CLI launch. Investigate: (a) what in `gui/` opens `cv2.VideoCapture` directly in the server process (find-cameras endpoint? robot-tab preview?); (b) whether the obs_stream reader can ever fall back to a direct open. Fix: every camera grab in the GUI server must be context-managed (`with cv2.VideoCapture(...) as cap:` or explicit try/finally with `cap.release()`); never hold camera state across requests.

### Trajectory-Replay Multi-Episode Loop

- [High] **Wire `trajectory_replay` into `lerobot-record` multi-episode flow.** Single-episode case works: `lerobot-teleoperate` and `lerobot-record`'s `record_loop` both check `teleop.is_exhausted` and break cleanly at end-of-trajectory. But the multi-episode path in `lerobot-record` calls `record_loop` again for the reset phase and again for each subsequent episode, all with the **same** teleop instance — which is still flagged exhausted, so each subsequent call breaks immediately. Two pieces to add: (1) reset phase should drive the robot back to rest position (e.g. call `move_to_rest_position(robot, profile.rest_position, duration_s=reset_time_s)`) instead of running `record_loop` with the exhausted teleop; (2) re-arm the teleop between episodes — either re-call `teleop.connect()` (current code-path is idempotent and re-reads the file) or add an explicit `teleop.reset()`. Tracked separately because the right shape depends on the broader "automated dataset iteration" flow the user is designing — wait for concrete signals before picking one.
