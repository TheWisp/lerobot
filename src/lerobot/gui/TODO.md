...
- [High] **A finished transfer leaves no recoverable outcome.** Observed on a
  real 8.4 GB upload: it completed and merged, the user reloaded the GUI some
  hours later, and the card was simply gone — with no way to tell whether it
  had succeeded, failed, or been cancelled. Answering the question required
  reading `~/.cache/lerobot/gui/hub_jobs/<job_id>.json` over SSH. Two causes,
  both needing a fix:
  1. **Nothing persists the outcome.** `gc_finished_hub_jobs` drops terminal
     jobs 30 minutes after `finished_at`, and `AppState.hub_jobs` is in-memory
     only, so a server restart erases the history immediately regardless. A
     multi-hour upload can finish, be GC'd, and leave the user with nothing.
  2. **The record the design doc promises was never built.** [docs/hub_transfers.md](docs/hub_transfers.md)
     specifies "JSONL analytics — one line appended to
     `~/.config/lerobot/gui/hub_transfers.jsonl` per terminal outcome
     (complete / failed / cancelled)". That file does not exist and nothing
     writes it. The doc reading as covered is why the gap went unnoticed.

  Implement the append-only JSONL at the worker's terminal write (it already
  owns that moment and survives server restarts), and give the tray a way to
  show recent finished transfers beyond the live window — a "recent" section,
  or at minimum a link that opens the record. The success case matters as much
  as the failure case: "did my 8 GB upload actually land?" must be answerable
  from the GUI.

- [Med] **Completion should snap the progress counters to the known totals.**
  `bytes_done_estimate` / `files_done_estimate` are documented _lower bounds_
  recovered from HF's bars, and on the classic-LFS path files the server
  already has never produce a bar at all. A successful 8.4 GB upload therefore
  finished reading `3.99 GB / 8.41 GB, 29/84 files` — a bar frozen near 47%
  on a transfer that fully succeeded, which reads as a partial upload. On a
  terminal `complete`, set the done counters to the totals the worker
  established up front; leave the lower-bound semantics for in-flight display.
