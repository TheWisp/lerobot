...
The gradient/rollout saliency runs **synchronously in the S1 inference thread**, every Nth inference (gradient ~40–55 ms backward · rollout ~22 ms forward-only; both share the GPU). **⚠️ Regression (2026-06-29 audit): it runs UNCONDITIONALLY on every HVLA-S1 run — not only when an overlay is open — so every deployment run pays this cost on the control thread AND leaks `.npz` dumps to disk. This is exactly the "a feature silently taxes every existing run" pattern; gate it before #42 ships.** Directions, in order:

- [**High — REGRESSION, blocks #42**] **Demand-gate the pass + flag the disk dump.** `_publish_aux` (`s1_inference.py`) runs the saliency every `_aux_every` (=3) steps **unconditionally**, and `_publish_saliency` writes a `.npz` grid dump to `outputs/saliency_dumps/<ts>/` on **every** publish — so every HVLA run, overlay or not, pays a ~40 ms DINOv2-unfrozen backward pass on the control thread + leaks files. Fixes: (1) `_overlay_method()` already attaches an `OverlayControlReader` whose `config()` is `None` when no worker is up (the documented demand signal) — wire it into `_publish_aux` to SKIP when the overlay is off; **first verify the off-signal is reliable** (control block absent/cleared on `/live/stop`, not stale). (2) Put the `.npz` dump behind an explicit debug flag (default off). (3) `_aux_every = 3` is a hardcoded placeholder hack — replace with a principled / demand-driven cadence. (4) Regression test: with no overlay control block, `_publish_aux` must NOT call `compute_input_saliency` / `compute_attention_rollout`. The #41 base had zero such cost.
