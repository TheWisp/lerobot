...
- [Mid] **File log misses ASGI / uvicorn errors**. `setup_logging()` in `gui/server.py` configures only the `lerobot.gui.*` namespace and sets `propagate=False`, so anything that raises in uvicorn / starlette / FastAPI handlers (unhandled ASGI exceptions, SSE generator failures, stream errors) prints only to the terminal that started the server — never to `~/.cache/huggingface/lerobot/gui/logs/server_*.log`. Surfaced when the gym-hil sim work hit a Python-3.10 `asyncio.TimeoutError` regression in the SSE keepalive: the file log showed only "Process exited rc=0", the actual traceback was console-only. Fix: also attach the file handler to `logging.getLogger("uvicorn.error")` and `logging.getLogger("uvicorn")`, or pass a `log_config` to `uvicorn.run` that mirrors stdout/stderr to the same rotating file. Inline TODO in `server.py:setup_logging` near the `propagate=False` line.
...
- [Low] **SIGKILL during HVLA shutdown leaks semaphores**. When stopping S1 inference (or any HVLA run) via the GUI's stop button, the SIGTERM → SIGKILL grace window in `gui/api/run.py` is too short for the full cleanup chain (robots, OpenCV cameras, RealSense, ObservationStream shm). RealSense in particular can take 1-2s to close. Visible as `resource_tracker: There appear to be N leaked semaphore objects` and `Process exited with code -9`. Cosmetic only — kernel reclaims the resources when the process dies — but noisy. Two fixes: (1) extend the grace window to ~15s for HVLA processes, (2) parallelize camera disconnects in `s1_process.py` cleanup, or call `obs_stream.cleanup()` _before_ camera disconnects so the shm is released first.
...
### Sim Visualization (longer-term)

- [Mid] **URDF skeleton viewer** for inspecting joint trajectories without spawning a sim window. Easiest path: push URDF + per-frame joint state to Rerun (which already has built-in URDF + joint visualization) and render via the existing Rerun web-viewer integration in `visualization_utils.py` — rather than rolling a `three.js` URDF viewer from scratch. Useful both for sim datasets (URDF guaranteed to exist) and any real-robot dataset that ships a URDF.
- [Low] Embedded simulator inside the GUI page (Mujoco WASM build). Significant integration cost (state sync, headless render, viewer parity); current "spawn gym-hil window alongside GUI" approach matches upstream HIL-SERL pattern. Don't pursue unless a concrete user-facing reason emerges.
...
- [Mid] **HVLA-on-sim is currently blocked**. Surveying gym-hil's task variants (the only sim env wired through the GUI today) shows two compounding incompatibilities with HVLA: (1) gym-hil ships only EE-delta action spaces — `Box([-1,-1,-1,0], [1,1,1,2], (4,))` — and HVLA emits joint positions in degrees; (2) gym-hil's Panda is single-arm 4-DOF, while HVLA targets bimanual SO-107 (14-DOF). Even an IK adapter (~150 LOC, joint targets → forward kinematics → EE pose → delta) wouldn't bridge a bimanual policy to a single-arm sim. Realistic paths forward: (a) **LeIsaac via EnvHub** — bimanual SO-101 in IsaacLab, requires a heavy CUDA + IsaacLab install but matches HVLA's robot shape natively, see `docs/source/envhub_leisaac.mdx`; (b) train a single-arm HVLA variant against a single-arm dataset, then drive gym-hil through an IK adapter; (c) wait for upstream gym-hil to ship a joint-space task variant. Until one of those lands, the GUI Run-tab Policy workflow stays real-robot only — it's deliberately untouched in the sim work.
