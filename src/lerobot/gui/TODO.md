...
The gradient/rollout saliency runs **synchronously in the S1 inference thread**, every Nth inference, on the **inference thread** (not the control loop). **⚠️ Regression (2026-06-29 audit): it runs UNCONDITIONALLY on every HVLA-S1 run, not only when an overlay is open. MEASURED 2026-06-29 (`flow_s1_no_s2` ckpt-40000, vits14, 4 cams @224): gradient 29 ms · rollout 23 ms · vs a normal 15-step inference 36 ms — saliency is CHEAPER than one inference (4 denoise steps vs 15). With `chunk_size=50` (~1.67 s of motion/inference @30 fps) the inference thread runs on a ~50× budget margin, so latency impact is negligible — NOT a perf/underrun problem. The real residual is the unbounded `.npz` disk dumps + the on-by-default principle. Worth gating before #42 ships, but as hygiene, not perf.** Directions, in order:

- [**Med — REGRESSION (hygiene; latency measured-negligible)**] **Demand-gate the pass + flag the disk dump.** `_publish_aux` (`s1_inference.py`) runs the saliency every `_aux_every` (=3) steps **unconditionally**, and `_publish_saliency` writes a `.npz` grid dump to `outputs/saliency_dumps/<ts>/` on **every** publish — so every HVLA run, overlay or not, recomputes saliency (measured 29 ms gradient, < one 36 ms inference; absorbed by the chunk-50 slack — no underruns) + leaks `.npz` files unboundedly. The disk leak + on-by-default are the real issues, **not latency**. Fixes: (1) `_overlay_method()` already attaches an `OverlayControlReader` whose `config()` is `None` when no worker is up (the documented demand signal) — wire it into `_publish_aux` to SKIP when the overlay is off; **first verify the off-signal is reliable** (control block absent/cleared on `/live/stop`, not stale). (2) Put the `.npz` dump behind an explicit debug flag (default off). (3) `_aux_every = 3` is a hardcoded placeholder hack — replace with a principled / demand-driven cadence. (4) Regression test: with no overlay control block, `_publish_aux` must NOT call `compute_input_saliency` / `compute_attention_rollout`. The #41 base had zero such cost.
