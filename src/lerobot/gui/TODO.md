...
- [Med] **Subprocess logs are ephemeral — no crash post-mortem (visibility gap).** The debug-vision standalone logs to a `tempfile.mkstemp` `/tmp/lerobot_debug_model_*.log` that `gui/api/run.py` **unlinks on unload/relaunch** (`_debug_output_path.unlink(missing_ok=True)`, ~line 533) — so when the model throws and the user unloads it, the traceback is GONE (this bit us diagnosing a live exception 2026-06-23: the model exited rc=0 but the user saw an exception we then couldn't recover). The teleop run's stdout is similarly SSE-only (in-memory, not persisted). **Fix:** write debug-model + run subprocess output to a durable path (e.g. `outputs/debug_model/<ts>.log`, keep last N), survive unload, and surface the path + last error in the GUI. Cheap, high-value observability. (Also: `/tmp` itself can be tmpfs/cleared on reboot — another reason not to rely on it.)
...
### Debug-Vision Models (SAM3 / picker)

- [ ] **Research further usages of SAM3, especially SAM 3.1.** Survey who's shipping it and what interaction patterns are worth borrowing for our overlay/picker. Known so far (2026-06, web search): Meta's Segment Anything Playground (text/click/example prompts + one-tap templates: pixelate faces & plates, spotlight, motion trails, magnify), Instagram Edits "Cutouts" (tap object → cut + track through a clip), Meta AI "Vibes" (effects on a chosen object), Facebook Marketplace "View in Room" (SAM3 **+ SAM3D** AR placement — same SAM3D path we already run), data-labeling pipelines, Conservation X Labs wildlife tracking. **SAM 3.1** (Mar 2026): 16→32 FPS, 8GB→4GB VRAM FP16, better accuracy via Object Multiplex + global reasoning — directly relevant to the "extra model is too heavy" worry; revisit whether 3.1 should replace 3 for the live path. Patterns to evaluate for adoption: hover-to-preview (highlight the mask under the cursor before clicking — cheap for us since the ~40 ms encode is cached, each hovered decode ≈14 ms), tap-to-track w/ per-object color (we already have the color list), one-widget-three-modes (text OR click OR example), and "pick the mask, the human names it" (annotation-tool flow — no captioner needed).
- [ ] **"Everything" / pick-from-scene picker.** No true automatic-mask mode today (`sam2_mask` only prompts a single center point; the whole-frame look people remember is `dino_features`' DINOv2 PCA heatmap, not instance masks). Add a real segment-everything via transformers' `mask-generation` pipeline (grid-of-points AMG) on the SAM2.1 backbone `sam2_mask` already loads — no new model. Flow: **Everything** → all objects as clickable colored masks → click one → it becomes a monitored-object row (you type/confirm the name; per annotation-tool convention, no captioner). Cost is ~40 ms encode once + batched 14 ms decodes ≈ sub-second, one-time on a frozen frame — not in the per-frame loop. Open decision (parked): whether a picked segment feeds the text list (needs a name) vs a geometric tracker (`sam3_tracker_video`, name-free) vs an image-exemplar on the SAM3 image model (`input_boxes`, image-mode only).
- [x] **SAM3 tracker model — researched + integrated (image, center-point).** The _geometric_ promptable-visual-segmentation head of the SAM3 family (SAM2 lineage): point clicks (positive/negative) + boxes → mask; the video variant propagates a clicked object via a memory bank. **Correction to an earlier note:** there is **no separate published tracker checkpoint** — the docstring ids (`danelcsb/sam3_tracker.1_*`, `facebook/sam3_tracker.1-*`) all **404**. SAM3 is a _unified_ model, so the tracker head ships **inside the gated `facebook/sam3` weights**; `Sam3TrackerModel.from_pretrained("facebook/sam3")` pulls it out via the class's `_checkpoint_conversion_mapping` (strips `tracker_model.`, ignores the memory/detector keys — benign "sam3_video → sam3_tracker" notice, 685 weights load). Measured (RTX 5090, fp16, single center point, 480×640): **~50 ms warm / ~300 ms cold ≈ 20 FPS**, faster than the concept/video models. Integrated as the `sam3_tracker` adapter (`Sam3TrackerAdapter`, mirrors `sam2_mask`'s center-point stopgap — a SAM **3.0** upgrade of it; SAM 3.1 is NOT loadable in our pinned env, see the next item). The adapter has a load-time guard that rejects a checkpoint loading with >8 missing params (= random weights). **Next:** a real click channel (the contract has none yet) so the seed isn't fixed to frame-center, then the video variant for click-once-propagate. Pairs with the picker item above.
- [x] **SAM 3.1 — found the weights, but NOT runnable in our env (2026-06-23).** The real 3.1 release IS published: **`facebook/sam3.1`** (gated; we have access) — but it ships **only Meta's raw checkpoint `sam3.1_multiplex.pt`** (3.5 GB), with **no HF-format safetensors**, so `from_pretrained` can't load it (3.0's `facebook/sam3` has both `.pt` and a converted `model.safetensors`; 3.1's conversion isn't uploaded). The community "conversions" (`strangervisionhf/sam3.1-st-bf16`, `research21/sam3.1`, `Comfy-Org/sam3.1`, `mlx-community/sam3.1-bf16`, …) **don't help**: they only re-saved the raw `.pt` as safetensors with **Meta's original key names** (`tracker.model.`, `detector.`), without remapping to the transformers scheme (`tracker_model.`, `vision_encoder.`, which also **splits** Meta's fused QKV). Loading one into `Sam3TrackerModel` gives an **all-MISSING model — random weights — garbage masks** (verified: 38.7% mask coverage vs the correct 8.3% from 3.0; HF logs MISSING/UNEXPECTED but does **not** raise — hence the adapter's load guard). On top of that, **transformers 5.3.0 doesn't implement 3.1's "Object Multiplex" architecture at all** ("multiplex" appears nowhere in its sam3 modeling code). **This is a transformers-integration gap, NOT a capability/hardware gap.** Meta's [official blog](https://ai.meta.com/blog/segment-anything-model-3/) covers SAM 3 **and** a SAM 3.1 update (dated 2026-03-27: "Object multiplexing", up to 16 objects/forward pass, 16→32 FPS on an H100) — 3.1 is real and official, and it **runs today via Meta's NATIVE SAM3 codebase + the `.pt`** (the blog links both the checkpoint and the GitHub codebase). It just doesn't run through HuggingFace `transformers`, which is the loader our whole debug-vision stack uses. **Two ways to get 3.1 here:** (a) wait for `transformers` to port the multiplex arch + a converted checkpoint (then a one-line `Sam3TrackerAdapter.CHECKPOINT` switch); or (b) bypass transformers and run Meta's native code path (separate, heavier integration — its own inference seam, not our adapter contract). Until then `sam3_tracker` runs SAM **3.0** (`facebook/sam3`), which transformers fully supports. (Earlier notes that called `sam3_tracker.1` "the shipped version" were wrong — that id 404s.)
- [ ] **Native SAM 3.1 adapter (bypass transformers).** transformers 5.3.0 can't load SAM 3.1 (see the finding above) — but Meta's native `sam3` codebase + the official `sam3.1_multiplex.pt` runs it today. Add a `sam3_1_native` adapter that loads 3.1 through Meta's own inference code behind the same `infer(frame)->rgba` seam, exactly like `cotracker3` already bypasses transformers via `torch.hub`. Isolate the install (its own venv, like SAM 3D Objects) so Meta's deps don't fight the main env; scope the dependency footprint before committing. Pays off for the many-object / 32-FPS regime; until then 3.0 via transformers is fine.
- [ ] **SAM3 video is slow + wrong for multi-view — per-camera sessions.** `Sam3VideoAdapter` keeps a SINGLE `self._session`, but the standalone calls `adapter.infer(frame)` once per active camera _without passing the camera identity_, so every view feeds one tracking session interleaved. That corrupts tracking (one memory bank fed N incompatible viewpoints) and adds churn. Fix: key sessions by camera (`infer(frame, cam=...)` or a per-camera adapter instance). Separately, image (`sam3`) being much faster than video is **mostly inherent**: detection-per-frame with the shared encoder (encode once + ~14 ms/concept) is lighter than the video model's per-frame memory-attention + per-object propagation, which also can't reuse the shared-encoder trick. Consider making `sam3` the default for the live overlay and reserving `sam3_video` for when temporal identity through occlusion is actually needed. **Update 2026-06-23:** the per-camera-session fix is DONE (`self._sessions` keyed by cam). But the new failure mode is **GPU OOM** — running `sam3_video` on all 4 cameras for ~9 min exhausted the 32 GB card, because each camera's streaming **memory bank grows** and there are now N of them. Fixes: bound/trim the SAM3-video memory bank (cap conditioning/recent frames per session), and/or default the live overlay to a **single camera** (the amodal toggle only needs `top` anyway). Workaround today = filter to one camera. A code-level guard (warn or auto-restrict when `sam3_video` + amodal run on >1 camera) would prevent the foot-gun.
- [ ] **SAM3-video: prune the streaming session + get CPU-vs-GPU placement right (the OOM root cause).** `Sam3VideoInferenceSession` stores per-frame state — `output_dict_per_obj[obj]["non_cond_frame_outputs"][frame_idx]` (maskmem features + masks) and `processed_frames[frame_idx]` (pixels) — and **never evicts** it (designed for a finite video, not an infinite stream). **Why it hits GPU:** `init_video_session` resolves `inference_state_device`/`video_storage_device` to `inference_device` when not passed (`processing_sam3_video.py:155`), and `Sam3VideoAdapter._start_session` passes ONLY `inference_device=cuda` — so the growing state defaults onto the **GPU** → CUDA OOM. **Proper fix (prune):** after each frame keep `cond_frame_outputs` (prompt anchors) + only the last **`num_maskmem`=7** non-cond frames the model actually attends to (plus the object-pointer horizon, ~16–30 frames ≈ the "few seconds" at ~14 fps — beyond that is pure waste). Constant memory forever, no accuracy loss. **CPU↔GPU best practice (perf):** keep the _pruned_ state on **GPU** — fastest (no per-frame transfers) and now tiny; that's the win. Setting `inference_state_device="cpu"` also stops the OOM but adds a **CPU↔GPU copy of the memory window every frame** — only worth it for a history longer than fits on GPU. So default to **prune + GPU**; offload to CPU only as a deliberate memory-for-latency trade, never blindly. **Measured (5090, real ring frame):** no prune = +691 MB GPU after 100 frames and climbing (~6–7 MB/frame = one 1008² fp16 input image, never freed → unbounded → OOM in minutes, ×N cameras); keep-last-30 (~2 s) plateaus FLAT at ~+0.2 GB; keep-last-7 (=`num_maskmem`) flat at ~+0.05 GB. No accuracy advantage beyond ~30 frames (model attends to last 7 dense + ~16 obj-pointers). Default retention ~32 frames (~2 s). **Leak audit (2026-06-23):** ran every integrated model for 80 steady-state frames (warmup past the fill-ramp), watching `torch.cuda.memory_allocated` — **all flat, no leaks**: the 6 stateless adapters (grounding_dino, dino_features, depth_anything, sam2_mask, sam3_tracker, sam3), `sam3_video` (with the prune above — the ONLY real unbounded leak, now bounded), `cotracker3` (bounded `_buf`/`_trails`, capped online window), and the FoundationPose worker (`track_one`×80 + `register`×10 flat). Re-run this check when adding a new debug-vision adapter (warm ≫ ramp, then watch allocated).
...
- [x] **SAM 3.1 integration** — attempted; landed on SAM **3.0**. `Sam3TrackerAdapter` (`sam3_tracker`) runs the geometric tracker head from `facebook/sam3` (3.0) and segments a center-point seed at ~20 FPS. SAM **3.1** turned out to be **not loadable in our pinned transformers 5.3.0** — official repo has no HF safetensors, community mirrors are raw-Meta-key (load as random weights), and the multiplex arch isn't implemented. Full diagnosis + the two things needed to unblock it are in the "SAM 3.1 — found the weights, but NOT runnable" item under "Debug-Vision Models" above. Not blocking the scan path.
- [ ] **Click channel for the tracker** (unblocks the picker + a useful tracker UX). The debug-vision `infer(frame)->rgba` contract has no click input, so `sam3_tracker`/`sam2_mask` are stuck at frame-center. Add: canvas click → coords through the overlay IPC → adapter seed point(s) (positive/negative). This is the prerequisite for "pick-from-scene" and for seeding the automatic scan from a human tap instead of a text concept.
...
- [ ] **Live pose tracking → use FoundationPose (feed-forward), NOT iterative render-compare.** We prototyped silhouette analysis-by-synthesis (fit the GLB pose so its rendered mask matches SAM3's) — works (synthetic recall ~1.0) but it's the **legacy iterative paradigm**: scipy Nelder-Mead re-renders the mesh ~50–150×/frame (the "~100 ms"), and silhouette-only fitting is ambiguous for symmetric objects (IoU caps ~0.85 even synthetically; cylinder/ring is worst-case). SOTA replaced this with **feed-forward** models. For our exact case (known mesh + live 6-DoF + occlusion completion) the targeted answer is **[FoundationPose](https://github.com/NVlabs/FoundationPose)** (NVIDIA, CVPR'24): model-based mode takes mesh (✓ SAM3D) + RGB (✓) + initial SAM3 mask (✓) + RealSense depth (✓) → real-time 6-DoF **tracking**, occlusion-robust via the model prior (= the amodal "hidden geometry"). Render-and-compare done right (its learned predecessor is [MegaPose](https://arxiv.org/pdf/2212.06870)). Keep SAM3 for the mask + the URDF viewer for display; FoundationPose just replaces the pose engine.
  - **Install DONE + validated (2026-06-23).** Recipe in [[reference_foundationpose_install]]: reused the SAM3D venv (already has torch 2.8+cu128 sm_120 + pytorch3d/nvdiffrast/kaolin), `uv pip install -r requirements.txt`, **reimplemented `mycpp.cluster_poses` in pure Python** (skips the C++/Boost/Eigen/cmake build entirely — no sudo, no CUDA build), weights from the HF mirror `ffurfaro/foundationpose-weights` (gdown quota-fails). **Synthetic round-trip passed**: render the ring at a known pose → `register` recovers it at **0.3 mm translation / silhouette IoU 0.996** (the "90.9° rotation" is just the ring's ambiguous axis-spin — visually identical). The recovered-pose mesh composited on the image IS the remap.
  - ⚠️ **ARCHITECTURE LIMITATION — FoundationPose must run as a SIDECAR (separate process + IPC), it cannot be an in-process debug-vision adapter.** A Python interpreter loads ONE package set, and FoundationPose's env is incompatible with the lerobot env on **two** axes: (1) torch **2.8** (SAM3D venv) vs **2.10** (lerobot env) — CUDA extensions are ABI-bound to their torch, so a build for one won't load in the other; (2) it needs **pytorch3d + nvdiffrast + kaolin** (heavy Blackwell-built CUDA exts) that the lerobot env doesn't have. So the toggle spawns a FoundationPose worker in the SAM3D venv and talks to it over **shared memory** (RGBD + mask in → amodal RGBA out — shared-mem because we stream ~8 MB/frame; sockets/files would copy per frame). The other debug-vision models (CoTracker3 via torch.hub, SAM3 via transformers) coexist in the lerobot env and need NO sidecar — only FoundationPose's CUDA-ext stack forces it. **Escapes, in increasing effort:** (a) unify the env — rebuild pytorch3d/nvdiffrast/kaolin against lerobot's torch 2.10 + run FP in-process (risks dependency hell; FP only tested on 2.8); (b) **ONNX/TensorRT-export** the refiner+scorer + replace nvdiffrast with a small CUDA rasterizer → sheds the heavy deps, runs anywhere (cleanest permanent fix); (c) ride SOTA toward pip-only feed-forward models (DA3-class) that are just adapters; (d) lean in — formalize a **typed shared-memory bus** of model workers (RGB/depth/masks/overlays/poses), each in its own env, GUI composites = the node-graph north-star in [[project_debug_vision_overlays]]. The standalone debug-vision subprocess is already sidecar #1; FoundationPose is sidecar #2.
  - [ ] **Env hygiene (SAM3D deps).** FoundationPose's Python deps were installed INTO the isolated `~/.cache/sam3d` venv (to avoid rebuilding the Blackwell GPU deps) — mild pollution. TODO: (a) smoke-test the SAM3D scan worker still runs (shares this venv); (b) eventually give FoundationPose its own venv that reuses the SAM3D GPU `.so`s, or pin/document the shared env. Low priority — SAM3D is reinstallable per [[reference_sam3d_blackwell_install]].
  - [ ] **Remapping the REAL ring needs depth + K + a metric mesh.** `register` is RGBD; our cached ring frame is RGB-only and the GLB scale is a fake 0.12 m, so we can't drive FoundationPose from the cache. Real path: RealSense RGBD capture (aligned depth + intrinsics) + SAM3 mask + a depth-anchored metric mesh (the do-as-i-do anchoring) → `register` then per-frame `track_one` → remap overlay. Interim test-on-existing-RGB option: monocular depth (metric-ambiguous; FP's nets prefer real metric depth → expect degraded).
  - [ ] **Depth model is outdated — upgrade to Depth Anything 3.** Our `depth_anything` adapter uses **V2** (`Depth-Anything-V2-Small-hf`). **[Depth Anything 3](https://depth-anything-3.github.io/)** (ByteDance-Seed, 2025-11-14, ICLR'26) is newer and a different class: a **DUSt3R-style multi-view 3D foundation model** that also does monocular, predicting **exponential depth + dense ray maps** (vs V2's affine-invariant disparity) for better _metric_ geometry + multi-view consistency (+10% ETH3D over V2). HF: `depth-anything/da3-{small,base,large,giant}` + `DA3Metric`/`DA3Mono`/`DA3Nested`. Caveat: ships as its own `depth-anything-3` pip package, NOT the transformers `AutoModelForDepthEstimation` API — separate install (its own adapter, like the `cotracker3`/native-SAM-3.1 pattern). Relevant twice over: (a) better monocular depth for the FoundationPose fallback; (b) as a feed-forward multi-view geometry model it overlaps the SOTA map below (could supply geometry/depth for the whole twin pipeline). The "depth-fusion pipeline" (RealSense aligned depth ⊕ learned depth estimate) the user wants would fuse camera depth with DA3.
  - SOTA map (for reference; most "real-time" claims are offline): feed-forward multi-view geometry without bundle adjustment — [Fast3R](https://fast3r-3d.github.io/) (251 FPS, 1000+ imgs/pass), [MV-DUSt3R+](https://mv-dust3rp.github.io/) (sparse views, 2 s); 3D point tracking — [SpatialTrackerV2](https://arxiv.org/abs/2507.12462) (feed-forward but **10–20 s/seq, not live**), [MVTracker](https://ethz-vlg.github.io/mvtracker/) (1–8 cams); depth-free 6-DoF — [RGBTrack](https://arxiv.org/pdf/2506.17119). These target _unknown/scene-level_ dynamic reconstruction — overkill for one known object; FoundationPose is the scoped fit.
