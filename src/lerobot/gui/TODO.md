...
- [Low] **`test_tff_subtracted_from_tau_ext` is flaky in CI.** Failed once on an unrelated branch (`assert False` on `maybe_report`, meaning `update()` did not register), passed locally and on a re-run of the same commit. Suspect the shape asymmetry in the fixture — `tff` is 7 elements while the other arrays are 8 — interacting with numpy broadcasting, which would make it environment-dependent rather than random. Worth ten minutes with `-p no:randomly` and a numpy version matrix before it costs someone a real debugging session.
