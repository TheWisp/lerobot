...
- [x] **SAM 3.1 integration** — done (image tracker). `Sam3TrackerAdapter` (`sam3_tracker`) loads the geometric SAM 3.1 tracker head out of `facebook/sam3` and segments a center-point seed at ~20 FPS. See the tracker research item under "Debug-Vision Models" above for measurements + the no-separate-checkpoint correction. Remaining tracker work (click channel, video propagation) is tracked there, not blocking the scan path.
