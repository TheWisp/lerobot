...
- [x] ~~**TOCTOU on `_active_process` and `_debug_process` between concurrent launches.**~~ Fixed 2026-05-13. `_launch_lock = asyncio.Lock()` added; `start_teleoperate` / `start_record` / `start_replay` / `start_hvla` now run their full `_ensure_no_active_process()` + (optional `_launch_debug_s2` under `_debug_lock`) + `_launch_subprocess(...)` sequence inside `async with _launch_lock:`. Regression test in `tests/gui/test_run.py::TestLaunchLockSerializes` spawns two concurrent teleoperate calls and asserts the second blocks on the lock until the first finishes, then receives 409 instead of silent orphaning.
...
- [High] **Foolproof shutdown-cleanup registry.** Today the server's shutdown hook is a hand-maintained list of cleanup calls (`_stop_debug_process`, `_terminate_active_process`, `cleanup_stale_streams`, `cleanup_in_process_resources`, `shutdown_prefetch_executor`). Adding a new module-level resource — preview cameras, recording robots, prefetch executor, anything yet to come — means remembering to wire it into `server.py::shutdown_event`. Easy to forget, and the omission is silent (the OS reclaims FDs on process death so cleanup never visibly fails). Shape options:
  ...
  **Orphan-subprocess failure mode (observed 2026-05-13).** A specific incarnation of this bug that bit hard: the GUI launches `lerobot-teleoperate` / `lerobot-record` / `lerobot-replay` subprocesses via `asyncio.create_subprocess_exec(..., start_new_session=True)`. The `start_new_session=True` is intentional — it detaches the child's process group so a Ctrl+C in the user's GUI terminal doesn't kill the workload mid-recording — but it also means **any abnormal GUI exit leaves the subprocess running**. Concrete case found: a `lerobot-teleoperate` subprocess was still alive almost 2 hours after the GUI server process that spawned it had exited, holding all four ttyACM ports and the cameras, blocking any subsequent launch. Only the graceful `shutdown_event` path calls `_terminate_active_process`; SIGKILL on the GUI (or any path that skips FastAPI lifespan) is silent and the subprocess survives indefinitely. Required to fix:
  - [x] ~~**Phase 1: parent-death detector**~~ (2026-05-13). `_set_pdeathsig_preexec` (Linux: `prctl(PR_SET_PDEATHSIG, SIGTERM)`) wired into both `_launch_subprocess` and `_launch_debug_s2` via `preexec_fn`. End-to-end test in `tests/gui/test_run.py::TestParentDeathDetector::test_child_dies_when_grandparent_killed` spawns a grandparent → child, SIGKILLs the grandparent, asserts the child dies. Handles the SIGKILL-on-GUI scenario; the cleanup-registry / lifespan migration below is still wanted for graceful-shutdown structure.
  - **Phase 2 (still TODO):** Whatever cleanup-registry mechanism we land must surface the active subprocess so a graceful shutdown path can terminate it explicitly (independent of the kernel-level pdeathsig fallback).
  - **Phase 3 (still TODO):** On GUI startup, scan for prior-launch zombies (e.g. by writing the subprocess PID into `~/.cache/huggingface/lerobot/gui/active_process.json` at launch and reading + killing on next startup if still alive and the heartbeat is stale). Belt-and-suspenders for the corner case where pdeathsig can't fire — e.g. if the child has already started ignoring SIGTERM.
...
- [Mid/Race] **`_preview_cameras` mutated without a lock**, in [src/lerobot/gui/api/robot.py](../api/robot.py). `_detect_and_open_cameras` (called via `/detect-cameras` in the executor) clears + repopulates the list; `_close_preview_cameras` (via `/stop-cameras` in the executor, AND via `cleanup_in_process_resources` from the shutdown sweep) clears it; `get_camera_frame` (`/camera-frame/{index}`) reads `_preview_cameras[index]` and immediately calls `camera.async_read()` on the handle. Concurrent `/camera-frame/N` polling at preview rate (~30 fps) against a `/stop-cameras` or `/detect-cameras` click races on three surfaces: (1) check-then-act on `len(_preview_cameras)` then `_preview_cameras[index]` — list cleared between the two lines → IndexError → HTTP 500; (2) `async_read` on a handle that another thread is mid-`disconnect()` — V4L2 / librealsense API misuse, can hang or crash the worker; (3) partial repopulate during detect → frame request sees a camera that isn't fully connected yet. Fix shape: add module-level `_preview_lock = threading.Lock()`; introduce `_locked` variants of the open/close functions (so callers already inside the lock — `_detect_and_open_cameras_locked` calling `_close_preview_cameras_locked` — don't redeadlock); wrap the entire `get_camera_frame` body (including `camera.async_read()`, since the disconnect path closes the underlying handle) in `with _preview_lock:`. Drafted once and then discarded mid-edit so the partial state didn't land — pick up clean next time. Note that this pattern is the same shape as the freshly-landed `_launch_lock` (TOCTOU on `_active_process`) and `_preview_lock` would be the third async/threadpool-shared-state lock after `_debug_lock` — fold all three into the broader "Foolproof shutdown-cleanup registry" pattern when that lands.
- [High/UX-hazard] **Save / Cancel buttons can scroll off-screen when the profile editor is longer than the viewport**, and switching tabs away does NOT warn about unsaved edits. Two distinct gaps:
  1. The "Save Changes" / "Cancel" controls are rendered inline at the bottom of the profile form. On a profile with many cameras + rest_position rows (typical for bi_so107), the form is taller than the viewport, and the user can edit a field, scroll up to see the result, and the decisive controls are no longer visible. They look like a confirmation isn't required — but it is, and the change is in-memory only.
  2. Clicking another tab (Run / Data / Model / …) silently abandons unsaved edits with no confirmation modal. Combined with (1), this is a real data-loss footgun: edit a field → scroll → switch tab → edits gone.

  Fixes: (a) sticky-position the Save/Cancel bar (`position: sticky; bottom: 0` on the action row + matching backdrop so it sits above content), so the controls are always reachable regardless of scroll position. (b) Hook the tab-switch and page-unload events: if `_isDirty()` returns true, show a "You have unsaved changes — Save / Discard / Cancel?" modal (or the browser-native `beforeunload` prompt for hard page navigations). The `_isDirty()` helper already exists in robot.js. (TODO upstreamable to main — the dirty-detection and Save/Cancel split is the same pattern across other LeRobot GUI tabs that have editable profiles.)
...
- [High] **Audit module-level singletons for thread-safety across all hot paths.** The 2026-05-14 GUI segfault traced to `video_utils._default_decoder_cache` returning the SAME `torchcodec.VideoDecoder` instance to multiple threads — its docstring claimed "thread-safe" but only the dict lookup was locked, not the decoder usage. Fixed at the GUI by routing decode through a single-worker executor (db6cd6ec0); but the underlying class is still misleadingly labelled and the issue would resurface anywhere else `LeRobotDataset` is shared across threads. Sweep: every `_default_*` / module-level singleton object across `lerobot.datasets`, `lerobot.motors`, `lerobot.cameras`, `lerobot.policies` — for each, ask "does this hold state that's not safe to touch from two threads at once?" and verify the call sites either own the lock or only use it from one thread. Suspected candidates: `_default_decoder_cache` (decoder share), `FeetechMotorsBus` (no internal lock — wrapped at call site via `LockedBus` for our two multi-threaded owner classes), ffmpeg / libav globals (init-once but no obvious foot-guns), `torch.hub` model cache. Resulting deliverable: a short doc listing each singleton + its actual concurrency contract + which call sites enforce it.
- [Mid] **Consolidate per-frame fetch to one HTTP request returning all cameras.** Today `app.js::loadAllFrames` fires N parallel `<img src=URL?camera=X>` requests per frame change (one per camera). The server's `_decode_and_cache` already decodes all cameras in one call — the multiple HTTP requests are pure UX→browser-rendering wiring, not a server-side need. Replace with `GET /api/datasets/{id}/episodes/{ep}/frame/{i}` returning `{cameras: {front: "<b64 jpeg>", ...}}` and have the frontend hydrate `<img src>` from `URL.createObjectURL(blob)` or `data:` URLs. Benefits: (1) eliminates by construction the same-frame request-coalescing problem that the single-worker fix currently papers over; (2) saves N−1 HTTP round-trips per scrub; (3) opens the door to multi-worker decode with singleflight if >2-client playback becomes a real need. Keep the per-camera endpoint around through the migration; remove once the frontend is fully on the new endpoint.
- [High] **Move draccus side-effect-on-import out of `lerobot.configs.parser` into a structural location.** `parser.py` runs two import-time side effects: monkey-patches `argparse.HelpFormatter._expand_help` and registers a decoder for `typing.Literal`. Every entry point that decodes a config has to remember to import `parser` somewhere along the dep chain or decoding crashes with cryptic errors like `Couldn't find a choice class for 'X'` (missing `@register_subclass` import) or `No decoding function for type typing.Literal[...]` (missing parser import). Bit HVLA's `s1_process.py` on 2026-05-13 when launching `bi_so107_follower_predictive` for the first time — fixed in 1e506c10b by adding explicit auto-discovery + `import lerobot.configs.parser`, but that's a workaround at one site. Structural fix: put the registration in `lerobot/robots/config.py` (always imported when `RobotConfig` is decoded) so the dependency becomes impossible to forget — anyone calling `draccus.decode(RobotConfig, ...)` necessarily imports `RobotConfig`, which now necessarily registers decoders. After the move, the explicit imports in `s1_process.py` and `gui/api/robot.py::_get_known_fields` become redundant. Pair with a smoke test in `tests/configs/` that decodes a `bi_so107_follower_predictive` config dict end-to-end so any future reorganization is caught by CI rather than a runtime crash for the user.
