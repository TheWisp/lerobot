...
- [x] **Cut `DepthEdgeOverlayProcessorStep` cost.** Done (2026-05-11). cProfile attribution found 70% of the cost was `np.percentile(valid_gradients, 95)` on a ~900k-sample gradient array — np.percentile fully sorts (O(N log N)). Replaced with a two-point `np.partition([idx_lo, idx_hi])` plus np.percentile's linear-interpolation formula (O(N), numerically equivalent). Also swapped `np.sqrt(gx**2 + gy**2)` for `cv2.magnitude(gx, gy)`. Measured: DepthEdge p50 11.52 → 9.53 ms (-1.99 ms), bit-identical output verified against a golden frame. The user trained on the existing overlay, so bit-identity was a hard requirement; the non-interpolated single partition was 2× faster but flipped 2 borderline pixels — rejected. Remaining DepthEdge cost is dominated by the partition itself (~3-4 ms) + the inner overlay-blend function (~3.7 ms); further wins would need a streaming-percentile estimator or hand-tuned blending, both diminishing returns vs. off-loop-add_frame.
