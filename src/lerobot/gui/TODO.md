...
- [Low] **Cross-origin iframes in screenshots**: html2canvas cannot read inside iframes whose origin differs from the GUI's. Today this hits the MeshCat URDF tile (served from `127.0.0.1:7000` while the GUI is on `:8000`) — it renders as a blank rectangle in bug-report screenshots and in the hidden Ctrl+Shift+S screenshot (`gui/static/screenshot.js`). For a real-pixel capture, the only browser-side options are (a) `getDisplayMedia()` (screen-sharing prompt, requires user gesture each time), or (b) ask the iframe origin to opt in via `postMessage`-based capture. For full ops-side debugging we already have `scripts/gui/screenshot_gui.py` (ffmpeg x11grab via CDP), but that's a separate path.
