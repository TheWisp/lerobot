...
class ProcessJobConfig:
    ...
    Pre: ``source_root`` is a readable LeRobotDataset; every treatment ``key`` (on
    ``objects`` and ``background_treatment``) is known. The worker parses this once at
    startup from ``LEROBOT_PROCESS_WORKER_CONFIG`` and never re-reads.
    """
    ...
    objects: list[dict]  # [{name, sign, treatment:{key,params}}] — each object's per-region treatment
    background_treatment: dict  # {key, params} applied to everything outside the objects
